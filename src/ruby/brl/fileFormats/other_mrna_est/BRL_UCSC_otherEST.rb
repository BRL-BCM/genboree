...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Other EST table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--otherESTFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC other EST table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --otherESTFile       | -r    => UCSC other EST file to convert
    --trackName             | -t    => Track name for other EST track.
                                       (type:subtype)
    --className             | -l    => Class name for other EST track.
  ...
  USAGE:
  BRL_UCSC_otherEST.rb -r xenoEst.txt.gz -t UCSC:xenoEst -l 'mRNA and EST' -i /users/ybai/work/Human_Project6/mRNA_EST/Other_EST -f xenoEst_LFF.txt -d /users/ybai/work/Human_Project6/mRNA_EST/Other_EST
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    otherESTFile = inputsHash['--otherESTFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{otherESTFile}"))
      $stderr.puts "WARNING: the file '#{otherESTFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    otherEST = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{otherESTFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        otherEST[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{otherEST[ff[10]]}".to_sym) if(otherEST[ff[10]] >= 1)
#        ff[10] = ("#{ff[10]}.#{otherEST[ff[10]]}") if(otherEST[ff[10]] >= 1)
        ...
        blockCount.times { |ii|  ### start from index 0
          ...
          f.print "#{className}\t#{ff[10]}.Block_#{blockNum}\t#{lffType}\t#{lffSubtype}\t#{ff[14]}\t#{tStarts[ii]+1}\t#{tStarts[ii]+blockSizes[ii]}\t#{abc}\t.\t1.0\t.\t.\t"
