...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC mouse Simple Repeats file to equivalent LFF version
# and output 1+ tracks
...
PROGRAM DESCRIPTION:
  ...
  COMMAND LINE ARGUMENTS:
    --trackName             | -t    => Track name for Polymorphisms:Simple Reapeats track.
                                       (type:subtype)
    --className             | -l    => Class name for Polymorphisms:Simple Repeats track.
  ...
  USAGE:
  i.e. BRL_UCSC_mouse_simpleRepeats.rb -t 'Polymorphisms:Simple Repeats'  -l 'Structural Variants' -i /users/ybai/work/Human_Project5/Simple_Repeats -f SimpleRepeats_LFF.txt -d /users/ybai/work/Human_Project5/Simple_Repeats -o simpleRepeat.txt.gz
"
end
    ...
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        f.print "#{className}\t#{ff[4]}\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]}\t#{ff[3]}\t+\t.\t#{ff[10]}\t.\t.\t"
