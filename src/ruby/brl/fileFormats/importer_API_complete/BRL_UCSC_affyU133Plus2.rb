...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Affy U133Plus2 table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--affyU133Plus2File', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Affy U133Plus2 table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --affyU133Plus2File       | -r    => UCSC affyU133Plus2 file to convert
    --trackName             | -t    => Track name for affyU133Plus2 track.
                                       (type:subtype)
    --className             | -l    => Class name for affyU133Plus2 track.
  ...
  USAGE:
  BRL_UCSC_affyU133Plus2.rb -r affyU133Plus2.txt.gz -t 'Affy:U133Plus2' -l 'Expression and Regulation' -i /users/ybai/work/Human_Project6/Expression_Regulation/Affy_U133Plus2 -f affyU133Plus2_LFF.txt -d /users/ybai/work/Human_Project6/Expression_Regulation/Affy_U133Plus2
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    affyU133Plus2File = inputsHash['--affyU133Plus2File'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{affyU133Plus2File}"))
      $stderr.puts "WARNING: the file '#{affyU133Plus2File}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    # CONVERT Affy U133Plus2 TO LFF RECORDS USING WHAT WE HAVE SO FAR
    affyU133Plus2 = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{affyU133Plus2File}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        affyU133Plus2[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{affyU133Plus2[ff[10]]}".to_sym) if(affyU133Plus2[ff[10]] >= 1)
        ...
        blockCount.times { |ii|  ### start from index 0
          ...
          f.print "matches=#{ff[1]}; misMatches=#{ff[2]}; repMatches=#{ff[3]}; nCount=#{ff[4]}; qNumInsert=#{ff[5]}; qBaseInsert=#{ff[6]}; tNumInsert=#{ff[7]}; tBaseI
sert=#{ff[8]}; " +
