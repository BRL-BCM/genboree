...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC mouse WSSD Coverage file to equivalent LFF version
...
def processArguments()
  optsArray = [
                ...
                ['--wssdCoverageFile', '-o', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Convert from UCSC WSSD Coverage file to equivalent LFF version.  

  COMMAND LINE ARGUMENTS:
    --trackName             | -t    => Track name for Mapping:WSSD Coverage track.
                                       (type:subtype)
    --className             | -l    => Class name for Mapping:WSSD Coverage track.
    ...
    --wssdCoverageFile       | -o    => UCSC WSSD Coverage file to convert
    --help                  | -h   => [optional flag] Output this usage
                                      info and exit.

  USAGE:
  i.e. BRL_UCSC_wssd_coverage.rb -t 'Mapping:WSSD Coverage'  -l 'Coverage' -i /users/ybai/work/Mouse_Project3/WSSD_Coverage -f wssd_coverage_LFF.txt -d /users/ybai/work/Mouse_Project3/WSSD_Coverage -o wssdCoverage.txt.gz
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    ...
    wssdCoverageFile = inputsHash['--wssdCoverageFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{wssdCoverageFile}"))
      $stderr.puts "WARNING: the file '#{wssdCoverageFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    wssdCoverage = Hash.new { |hh, kk| hh[kk] = 0 }

    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{wssdCoverageFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[1].strip!  # name 
        wssdCoverage[ff[1]] += 1
        name = ("#{ff[1]}.#{wssdCoverage[ff[1]]}".to_sym) if(wssdCoverage[ff[1]] >= 1)
        ...
        f.print "#{className}\t#{name}\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]}\t#{ff[3]}\t+\t.\t1.0\t.\t.\t"
