...
PROGRAM DESCRIPTION:
  ...
  USAGE:
  i.e. BRL_UCSC_mgiqtl_mm8.rb -t 'Mapping:MGI QTL'  -l 'Phenotype Disease Ass' -i /users/ybai/work/Mouse_Project3/MGI_QTL -f mgi_qtl_LFF.txt -d /users/ybai/work/Mouse_Project3/MGI_QTL -o jaxQTL.txt.gz
"
end
    ...
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[7] = ff[7].to_sym  #mgiID
        ff[8] = ff[8].to_sym  #description
        ff[9] = ff[9].to_f  #cMscore
        ff[3].strip!  # name 
        mgiQtl[ff[3]] += 1
        ...
        f.print "marker=#{ff[6]}; mgiID=#{ff[7]}; description=#{ff[8]}; cMscore=#{ff[9]}"
        # sequence (none)
        f.print "\t.\t"
