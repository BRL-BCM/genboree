...
\margl1440\margr1440\vieww19980\viewh13820\viewkind0
...
\};\
\
INSERT INTO data_acquisition_complete (db_name, remote_track_name, class_name,host,file_to_download,d_directory_output,converter_name,file_to_output,c_directory_output,  database_upload, url_label, url, ucsc_description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);\
\
SELECT data_acquisition_complete.remote_track_name, data_acquisition_complete.database_upload \
FROM data_acquisition_complete\
WHERE data_acquisition_complete.db_name = 'USER_SELECTED_DATABASE_NAME (i.e. hg18)' AND data_acquisition_complete.database_upload = 'USER_INPUT_DATABASE(i.e. hg18_MappingAndSequencing)';\
\
