...
# ###################################################################################################################################
# Program: A downloader which can download the files from Ensembl given info about the external host, the path, the requested file list, etc. 
...
PROGRAM DESCRIPTION:
  Download requested source file(s) given a particular assembly version of a species available from Ensembl.

  COMMAND LINE ARGUMENTS:
    --hostFile              | -o    => Ensembl host address to contact
  ...
  USAGE: ruby BRL_Ensembl_downloader.rb -o host -a assembly_for_a_species -f fileList -d directoryOutputFileGo-e email_address
  i.e. BRL_Ensembl_downloader.rb -o ftp.ensembl.org -a homo_sapiens_core_47_36i -f *.gz -d /users/ybai/work/Project1/test_Downloader -e ybai@ws59.hgsc.bcm.tmc.edu
"
end
      ...
      timeout(200){
        ftp = Net::FTP.open(hostFile) do |ftp|
          ...
          begin
            ...
            ftp.chdir("pub/current_mysql/#{current_assembly}")
