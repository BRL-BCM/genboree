...
PROGRAM DESCRIPTION:
  ...
  USAGE:
  BRL_UCSC_fosmidlff.rb -r fosEndPairs.txt.gz -t Fosmid:EndPairs -l 'End Pairs' -i /users/ybai/work/Project1/test_Downloader -f fosEndPairs_LFF.txt -d /users/ybai/work/Project1/test_Converter
"
end
        ...
        lfCount.times { |ii|  
          ...
          while(read_index <= lfCount - 1)
            if(read_index != ii and strand[read_index] != strand[ii]) ## if not current read and not the same sequenced end
              ...
              f.print "bin=#{bin}; pslTable=#{pslTable}; lfCount=#{lfCount}; "
