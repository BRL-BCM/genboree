...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC mRNA table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--allmRNAFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC mRNA table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --allmRNAFile       | -r    => UCSC mRNA file to convert
    --trackName             | -t    => Track name for mRNA track.
                                       (type:subtype)
    --className             | -l    => Class name for mRNA track.
  ...
  USAGE:
  BRL_UCSC_allmRNA.rb -r all_mrna.txt.gz -t UCSC:mRNA -l 'mRNA and EST' -i /users/ybai/work/Human_Project6/mRNA_EST/All_mRNA -f allmRNA_LFF.txt -d /users/ybai/work/Human_Project6/mRNA_EST/All_mRNA
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    allmRNAFile = inputsHash['--allmRNAFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{allmRNAFile}"))
      $stderr.puts "WARNING: the file '#{allmRNAFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    allmRNA = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{allmRNAFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        allmRNA[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{allmRNA[ff[10]]}".to_sym) if(allmRNA[ff[10]] >= 1)
