...
def processArguments()
  optsArray = [
                ['--indelTCAGFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Database of Genomic indels variation table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --indelTCAGFile         | -r    => UCSC indel file to convert
    --trackName             | -t    => Track name for Indels:TCAG.v7 track.
                                       (type:subtype)
  ...
  USAGE:
  BRL_UCSC_INDELTCAG.rb -r indel.hg18.v7.mar.2009.txt -t Indels:TCAG.v7 -l 'Structural Variants' -i /users/ybai/work/Variants_TCAG -f indel.hg18.v7_LFF.txt -d /users/ybai/work/Variants_TCAG
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    indelTCAGFile = inputsHash['--indelTCAGFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{indelTCAGFile}"))
      $stderr.puts "WARNING: the file '#{indelTCAGFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    # CONVERT indelTCAG TO LFF RECORDS USING WHAT WE HAVE SO FAR
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{indelTCAGFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        # Chop it up
        # VariationID	Landmark	Chr	Start	End	VariationType	LocusChr	LocusStart	LocusEnd	Reference	PubMedID	Method/platform	Gain	Loss	TotalGainLossInv	Frequency	SampleSize Genes
        ff = line.chomp.split(/\t/)
        ...
        if("#{ff[6]}" != '') 
          ff[6] = ff[6].to_sym	#LocusChr
        end
        if("#{ff[7]}" != '') 
          ff[7] = ff[7].to_i	#LocusStart
        end
        if("#{ff[8]}" != '') 
          ff[8] = ff[8].to_i	#LocusEnd
        end
        if("#{ff[9]}" != '') 
          ff[9] = ff[9].to_sym	#Reference
        end
        if("#{ff[10]}" != '') 
          ff[10] = ff[10].to_i	#PubMedID
        end
        if("#{ff[11]}" != '') 
          ff[11] = ff[11].to_sym	#Method/Platform
        end
        if("#{ff[12]}" != '') 
          ff[12] = ff[12].to_i	#Gain
        end
        if("#{ff[13]}" != '') 
          ff[13] = ff[13].to_i	#Loss
        end
        if("#{ff[14]}" != '') 
          ff[14] = ff[14].to_i	#TotalGainLossInv
        end
        if("#{ff[15]}" != '') 
          ff[15] = ff[15].to_i	#Frequency
        end
        if("#{ff[16]}" != '') 
          ff[16] = ff[16].to_sym	#SampleSize
        end
        if("#{ff[17]}" != '') 
          ff[17] = ff[17].to_sym	#Genes
        end
        ...
        f.print "#{className}\t#{ff[0]}\t#{lffType}\t#{lffSubtype}\t#{ff[2]}\t#{ff[3]+1}\t#{ff[4]}\t+\t.\t1.0\t.\t.\t"
        ## attributes in order of useful information (in LFF anyway)
        f.print "VariationID=#{ff[0]}; Landmark=#{ff[1]}; variationType=#{ff[5]}; LocusChr=#{ff[6]}; LocusStart=#{ff[7]+1}; LocusEnd=#{ff[8]}; Reference=#{ff[9]}; PubMedID=#{ff[10]}; Method/Platform=#{ff[11]}; Gain=#{ff[12]}; Loss=#{ff[13]}; TotalGainLossInv=#{ff[14]}; Frequency=#{ff[15]}; SampleSize=#{ff[16]}; Genes=#{ff[17]}"
        # sequence (none)
        f.print "\t.\t"
