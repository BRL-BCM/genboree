...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Affy GNF1H table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--affyGNF1HFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Affy GNF1H table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --affyGNF1HFile       | -r    => UCSC Affy GNF1H file to convert
    --trackName             | -t    => Track name for Affy GNF1H track.
                                       (type:subtype)
    --className             | -l    => Class name for Affy GNF1H track.
  ...
  USAGE:
  BRL_UCSC_affyGNF1H.rb -r affyGnf1h.txt.gz -t AFFY:GNF1H -l 'Expression and Regulation' -i /users/ybai/work/Human_Project6/Expression_Regulation/Affy_GNF1H -f affyGnf1h_LFF.txt -d /users/ybai/work/Human_Project6/Expression_Regulation/Affy_GNF1H
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    affyGNF1HFile = inputsHash['--affyGNF1HFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{affyGNF1HFile}"))
      $stderr.puts "WARNING: the file '#{affyGNF1HFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    affyGNF1H = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{affyGNF1HFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        affyGNF1H[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{affyGNF1H[ff[10]]}".to_sym) if(affyGNF1H[ff[10]] >= 1)
        ...
        blockCount.times { |ii|  ### start from index 0
          ...
          f.print "#{className}\t#{ff[10]}.Block_#{blockNum}\t#{lffType}\t#{lffSubtype}\t#{ff[14]}\t#{tStarts[ii].to_i+1}\t#{tStarts[ii].to_i+1+blockSizes[ii].to_i}\t#{ff[9]}\t.\t1.0\t.\t.\t"
          ...
          f.print "matches=#{ff[1]}; misMatches=#{ff[2]}; repMatches=#{ff[3]}; nCount=#{ff[4]}; qNumInsert=#{ff[5]}; qBaseInsert=#{ff[6]}; tNumInsert=#{ff[7]}; tBaseInsert=#{ff[8]}; " +
                "qSize=#{ff[11]}; tName=#{ff[14]}; tSize=#{ff[15]}; qStart=#{qStarts[ii].to_i+1};" +
