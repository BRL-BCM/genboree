...
PROGRAM DESCRIPTION:
  Convert from UCSC multiple files (MGI MOUSE QTL) to equivalent LFF version. The other flag must be turned off (-1) while one flag is set on (1)  
  ...
  COMMAND LINE ARGUMENTS:
    ...
    --trackName             | -t    => Track name for RGD:MGI MOUSE QTL track.
                                       (type:subtype)
    --className             | -l    => Class name for RGD:MGI MOUSE QTL track.
      ...
      open("#{cDirectoryOutput}/#{fileToOutput}", 'w') do |f|
        if(inputsHash.key?('--jaxQtlAsIsFile'))
            ...
            reader.each { |line|
              ...
              ff[4].strip!  # name 
              jaxQtlAsIs[ff[4]] += 1
              mgiNam = ("#{ff[4]}.#{jaxQtlAsIs[ff[4]]}".to_sym) if(jaxQtlAsIs[ff[4]] > 1)
              ...
              f.print "#{className}\t#{mgiNam}.jaxQtlAsIs\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+1}\t#{ff[3]}\t+\t.\t1.0\t.\t.\t"
              ## attributes in order of useful information (in LFF anyway)
              f.print "url=http://www.informatics.jax.org/searchtool/Search.do?query=#{ff[4]}&submit=Quick+Search; annotationColor=#FF0000"
              # sequence (none)
              f.print "\t.\t"
        ...
        if(inputsHash.key?('--jaxQtlPaddedFile'))
          ...
          begin
            ...
            reader.each { |line|
              ...
              ff[4].strip!  # name 
              jaxQtlPadded[ff[4]] += 1
              mgiName = ("#{ff[4]}.#{jaxQtlPadded[ff[4]]}".to_sym) if(jaxQtlPadded[ff[4]] > 1)

              # Dump each linked feature as LFF
              ### class, name, type, subtype, entry point(chr), start, stop, strand, phase, score, qStart, qStop, attri_comments, seq, free_comments
              f.print "#{className}\t#{mgiName}.jaxQtlPadded\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+1}\t#{ff[3]}\t+\t.\t1.0\t.\t.\t"
              # attri_comments
              f.print "url=http://www.informatics.jax.org/searchtool/Search.do?query=#{ff[4]}&submit=Quick+Search; annotationColor=#0000FF"
              # sequence (none)
              f.print "\t.\t"
              # summary (free form comments)
              f.print "."
