...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC delHinds2 table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--delHinds2File', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC delHinds2 table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --delHinds2File       | -r    => UCSC delHinds2 file to convert
    --trackName             | -t    => Track name for delHinds2 track.
                                       (type:subtype)
    --className             | -l    => Class name for delHinds2 track.
  ...
  USAGE:
  BRL_UCSC_delHinds2.rb -r delHinds2.txt.gz -t DEL:Hinds2 -l 'Structural Variants' -i /users/ybai/work/Structural_Variants/converter_delHinds2_new -f delHinds2_LFF.txt -d /users/ybai/work/Structural_Variants/converter_delHinds2_new
"
end
    ...
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        f.print "#{className}\t#{ff[4]}\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+1}\t#{ff[3]}\t+\t.\t#{ff[5]}\t.\t.\t"
        # attri_comments (none)
