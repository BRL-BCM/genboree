...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC spliced EST table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--splicedESTFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC spliced EST table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --splicedESTFile       | -r    => UCSC spliced EST file to convert
    --trackName             | -t    => Track name for spliced EST track.
                                       (type:subtype)
    --className             | -l    => Class name for spliced EST track.
  ...
  USAGE:
  BRL_UCSC_splicedEST.rb -r chr1_intronEst.txt.gz -t 'UCSC:Spliced EST' -l 'mRNA and EST' -i /users/ybai/work/Human_Project6/mRNA_EST/Spliced_EST -f chr1_intronEst__LFF.txt -d /users/ybai/work/Human_Project6/mRNA_EST/Spliced_EST
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    splicedESTFile = inputsHash['--splicedESTFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{splicedESTFile}"))
      $stderr.puts "WARNING: the file '#{splicedESTFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    splicedEST = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{splicedESTFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        splicedEST[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{splicedEST[ff[10]]}".to_sym) if(splicedEST[ff[10]] >= 1)
