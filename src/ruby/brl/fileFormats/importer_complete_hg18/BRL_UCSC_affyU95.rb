...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Affy U95 table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--affyU95File', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Affy U95 table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --affyU95File       | -r    => UCSC Affy U95 file to convert
    --trackName             | -t    => Track name for Affy U95 track.
                                       (type:subtype)
    --className             | -l    => Class name for Affy U95 track.
  ...
  USAGE:
  BRL_UCSC_affyU95.rb -r affyU95.txt.gz -t AFFY:U95 -l 'Expression and Regulation' -i /users/ybai/work/Human_Project6/Expression_Regulation/Affy_U95 -f affyU95_LFF.txt -d /users/ybai/work/Human_Project6/Expression_Regulation/Affy_U95
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    affyU95File = inputsHash['--affyU95File'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{affyU95File}"))
      $stderr.puts "WARNING: the file '#{affyU95File}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    affyU95 = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{affyU95File}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        affyU95[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{affyU95[ff[10]]}".to_sym) if(affyU95[ff[10]] >= 1)
