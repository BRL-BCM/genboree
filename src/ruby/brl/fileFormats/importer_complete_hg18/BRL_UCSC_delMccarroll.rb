...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC delMccarroll table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--delMccarrollFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC delMccarroll table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --delMccarrollFile       | -r    => UCSC delMccarroll file to convert
    --trackName             | -t    => Track name for delMccarroll track.
                                       (type:subtype)
    --className             | -l    => Class name for delMccarroll track.
  ...
  USAGE:
  BRL_UCSC_delMccarroll.rb -r delMccarroll.txt.gz -t DEL:Mccarroll -l 'Structural Variants' -i /users/ybai/work/Structural_Variants/converter_delMccarroll_new -f delMccarroll_LFF.txt -d /users/ybai/work/Structural_Variants/converter_delMccarroll_new
"
end
    ...
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        f.print "#{className}\t#{ff[4]}\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+1}\t#{ff[3]}\t+\t.\t1.0\t.\t.\t"
        #attri_comments
        f.print "Landmark=#{ff[1]}:#{ff[2]+1}..#{ff[3]}"
        # sequence (none)
        f.print "\t.\t"
