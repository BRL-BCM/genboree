     ...
     # Go through each line
      reader.each { |line|
        ...
        blockCount.times { |ii|  ### start from index 0
          ...
          f.print "#{className}\t#{ff[10]}.Block_#{blockNum}\t#{lffType}\t#{lffSubtype}\t#{ff[14]}\t#{tStarts[ii]+1}\t#{tStarts[ii]+1+blockSizes[ii]}\t#{ff[9]}\t.\t1.0\t.\t.\t"
          ...
          f.print "matches=#{ff[1]}; misMatches=#{ff[2]}; repMatches=#{ff[3]}; nCount=#{ff[4]}; qNumInsert=#{ff[5]}; qBaseInsert=#{ff[6]}; tNumInsert=#{ff[7]}; tBaseInsert=#{ff[8]}; " +
                "qSize=#{ff[11]}; tName=#{ff[14]}; tSize=#{ff[15]}; qStart=#{qStarts[ii]+1};" +
