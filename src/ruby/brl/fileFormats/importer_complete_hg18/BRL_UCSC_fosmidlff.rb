...
PROGRAM DESCRIPTION:
  ...
  COMMAND LINE ARGUMENTS:
    ...
    --trackName             | -t    => Track name for fosEndPairs track.
                                       (type:subtype)
    --className             | -l    => Class name for fosEndPairs track.
  ...
  USAGE:
  BRL_UCSC_fosmidlff.rb -r fosEndPairs.txt.gz -t Fosmid:EndPairs -l 'End Pairs' -i /users/ybai/work/Project1/Fosmid -f fosEndPairs_LFF.txt -d /users/ybai/work/Project1/Fosmid
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    ...
    unless(File.size?("#{cDirectoryInput}/#{fosEndPairsFile}"))
    ...
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{fosEndPairsFile}")
    line = nil
    begin
        ...
        lfCount.times { |ii|  
          ...
          while(read_index <= lfCount - 1)
            if(read_index != ii and strand[read_index] != strand[ii]) ## if not current read and not the same sequenced end
              ...
              f.print "#{className}\t#{ff[4]}\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{lfStarts[ii].to_i+1}\t#{lfStarts[ii].to_i+1+lfSizes[ii].to_i}\t#{strand[ii]}\t.\t#{ff[5]}\t.\t.\t"
              # attributes in order of useful information (in LFF anyway)
              ### lfCount, readName, mateName, mateStart, mateStop, mateChr
              f.print "lfCount=#{lfCount}; "
              f.print "readName=#{lfNames[ii].to_sym}; mateName=#{lfNames[read_index].to_sym}; " +
                "mateStart=#{lfStarts[read_index].to_i+1}; mateStop=#{lfStarts[read_index].to_i + 1 + lfSizes[read_index].to_i}; mateChr=#{ff[1]} "
            end
            read_index += 1	
          end	 
