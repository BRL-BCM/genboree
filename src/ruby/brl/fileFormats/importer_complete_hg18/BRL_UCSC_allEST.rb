...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC EST table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--allESTFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC EST table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --allESTFile       | -r    => UCSC EST file to convert
    --trackName             | -t    => Track name for EST track.
                                       (type:subtype)
    --className             | -l    => Class name for EST track.
  ...
  USAGE:
  BRL_UCSC_allEST.rb -r chr1_est.txt.gz -t UCSC:EST -l 'mRNA and EST' -i /users/ybai/work/Human_Project6/mRNA_EST/All_EST -f chr1_est_LFF.txt -d /users/ybai/work/Human_Project6/mRNA_EST/All_EST
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    allESTFile = inputsHash['--allESTFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{allESTFile}"))
      $stderr.puts "WARNING: the file '#{allESTFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    allEST = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{allESTFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        allEST[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{allEST[ff[10]]}".to_sym) if(allEST[ff[10]] >= 1)
