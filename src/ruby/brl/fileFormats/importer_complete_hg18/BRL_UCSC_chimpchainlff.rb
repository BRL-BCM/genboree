...
PROGRAM DESCRIPTION:
  ...
  USAGE:
  BRL_UCSC_chimpchainlff.rb -r chr1_chainPanTro2.txt.gz -t Alignment:Chain -l 'Comparative Genomics' -i /users/ybai/work/Project1/Chain -s Human -q Chimp -f chr1_chainPanTro2_LFF.txt -d /users/ybai/work/Project1/Chain
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    ...
    ## ==========> Phase I:  check the number of unique chromosomes (including _random ones) from the input file and sort regular ones in a numeric order
      ...
      #modifed recently for handling input query random chromosome is none
      if(numRandomChrs > 0)
    ...
    begin
       ...
       reader.each { |line|
         ...
         f.print "#{className}\t#{qspecies}.#{ff[6]}\t#{lffType}\t#{lffSubtype}\t#{tChromosome}\t#{ff[4]+1}\t#{ff[5]}\t#{ff[8]}\t.\t#{ff[1]}\t#{ff[9]+1}\t#{ff[10]}\t"
         # attributes in order of useful information (in LFF anyway)
         ### bin, qSize, tSize 
         f.print "qChr=#{qChromosome}; qSize=#{ff[7]}; tSize=#{ff[3]};"
