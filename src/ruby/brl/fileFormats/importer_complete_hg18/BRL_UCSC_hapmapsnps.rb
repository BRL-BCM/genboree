...
class MyConverter
  ...
  def convert(inputsHash) 
   ...
   #         ff[12] = ff[12].to_i  #heteroCount
            ...
            f.print "#{className}\t#{ff[4]}.hapmapSnpsCEU\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+1}\t#{ff[3]+1}\t#{ff[6]}\t.\t#{ff[5]}\t.\t.\t"
            # attributes in order of useful information (in LFF anyway)
            f.print "observed=#{ff[7]}; allele1=#{ff[8]}; homoCount1=#{ff[9]}; allele2=#{ff[10]}; homoCount2=#{ff[11]}; heteroCount=#{ff[12]}; annotationColor=#00ff00"
            # sequence (none)
            f.print "\t.\t"
      ...
      if(inputsHash.key?('--hapmapSnpsCHBFile'))
        ...
        begin
          ...
          reader.each { |line|
            ...
            # Dump each linked feature as LFF
            ### class, name, type, subtype, entry point(chr), start, stop, strand, phase, score, qStart, qStop, attri_comments, seq, free_comments
            f.print "#{className}\t#{ff[4]}.hapmapSnpsCHB\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+1}\t#{ff[3]+1}\t#{ff[6]}\t.\t#{ff[5]}\t.\t.\t"
            # attributes in order of useful information (in LFF anyway)
      ...
      if(inputsHash.key?('--hapmapSnpsJPTFile'))
        ...
        begin
          ...
          reader.each { |line|
            ...
            # Dump each linked feature as LFF
            ### class, name, type, subtype, entry point(chr), start, stop, strand, phase, score, qStart, qStop, attri_comments, seq, free_comments
            f.print "#{className}\t#{ff[4]}.hapmapSnpsJPT\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+1}\t#{ff[3]+1}\t#{ff[6]}\t.\t#{ff[5]}\t.\t.\t"
            # attributes in order of useful information (in LFF anyway)
      ...
      if(inputsHash.key?('--hapmapSnpsYRIFile'))
        ...
        begin
          ...
          reader.each { |line|
            ...
            # Dump each linked feature as LFF
            ### class, name, type, subtype, entry point(chr), start, stop, strand, phase, score, qStart, qStop, attri_comments, seq, free_comments
            f.print "#{className}\t#{ff[4]}.hapmapSnpsYRI\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+1}\t#{ff[3]+1}\t#{ff[6]}\t.\t#{ff[5]}\t.\t.\t"
            # attributes in order of useful information (in LFF anyway)
