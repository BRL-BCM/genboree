...
# ##############################################################################
# PURPOSE
# Simple: convert from UCSC chimp chain table to equivalent LFF version
# ##############################################################################
...
PROGRAM DESCRIPTION:
  Converts from UCSC chimp chain table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --chimpChainFile        | -r    => UCSC chimp chain file to convert
    --trackName             | -t    => Track name for chimp chain track.
                                       (type:subtype)
  ...
  USAGE:
  BRL_UCSC_chimpchainlff.rb -r chr1_chainPanTro2.txt.gz -t Alignment:Chain -s Human -q Chimp >chr1_chainPanTro2_LFF.txt
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    ...
    unless(File.size?(chimpChainFile))
    ...
    # Open the file
    reader1 = BRL::Util::TextReader.new(chimpChainFile)
    reader = BRL::Util::TextReader.new(chimpChainFile)
    line = nil
    ...
    begin
       ...
       reader.each { |line|
         ...
         print "Comparative Genomics\t#{qspecies}.#{ff[6]}\t#{lffType}\t#{lffSubtype}\t#{tChromosome}\t#{ff[4]}\t#{ff[5]}\t#{ff[8]}\t.\t#{ff[1]}\t#{ff[9]}\t#{ff[10]}\t"
         # attributes in order of useful information (in LFF anyway)
         ### bin, qSize, tSize 
         print "bin=#{bin}; qSize=#{ff[7]}; tSize=#{ff[3]};"
