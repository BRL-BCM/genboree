...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Affy U74B table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--affyU74BFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Affy U74B table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --affyU74BFile       | -r    => UCSC Affy U74B file to convert
    --trackName             | -t    => Track name for Affy U74B track.
                                       (type:subtype)
    --className             | -l    => Class name for Affy U74B track.
  ...
  USAGE:
  BRL_UCSC_affyU74B.rb -r affyGnfU74B.txt.gz -t AFFY:U74B -l 'Expression and Regulation' -i /users/ybai/work/Project6/Expression_Regulation/Affy_U74B -f affyGnfU74B_LFF.txt -d /users/ybai/work/Project6/Expression_Regulation/Affy_U74B
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    affyU74BFile = inputsHash['--affyU74BFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{affyU74BFile}"))
      $stderr.puts "WARNING: the file '#{affyU74BFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    affyU74B = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{affyU74BFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[4].strip!  # name 
        affyU74B[ff[4]] += 1  
        ff[4] = ("#{ff[4]}.#{affyU74B[ff[4]]}".to_sym) if(affyU74B[ff[4]] >= 1)
