...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC QTL files (Human Quantitative Trait Locus from RGD) to equivalent LFF version
...
PROGRAM DESCRIPTION:
  Convert from UCSC qtl file (Human Quantitative Trait Locus from RGD) to equivalent LFF version.  

  COMMAND LINE ARGUMENTS:
    --trackName             | -t    => Track name for RGD:QTL track.
                                       (type:subtype)
    --className             | -l    => Class name for RGD:QTL track.
  ...
  USAGE:
  i.e. BRL_UCSC_qtl.rb -t RGD:QTL -l 'Phenotype Disease Ass' -i /users/ybai/work/Project3/RGD_QTL -f RGD_QTL_LFF.txt -d /users/ybai/work/Project3/RGD_QTL -o rgdQtl.txt.gz -p rgdQtlLink.txt.gz 
"
end
    ...
    open("#{cDirectoryOutput}/#{fileToOutput}", 'w') do |f|
      ...
      begin
        ...
        reader.each { |line|
          ...
          linkDescription = rgdQtlLink.key?(ff[4]) ? rgdQtlLink[ff[4]].description : ''
          linkId = rgdQtlLink.key?(ff[4]) ? rgdQtlLink[ff[4]].id : ''
          if(rgdQtlLink.key?(ff[4])) # if optional file is provided 
            if("#{ff[4]}" == "#{rgdQtlLink[ff[4]].name}") # if there is a description for this name in Link file
              f.print "#{linkDescription}\t"
            else
              f.print "#{ff[4]}\t"
            end
          else # optional file is not provided
              f.print "#{ff[4]}\t"
          end

          f.print "#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]}\t#{ff[3]}\t+\t.\t1.0\t.\t.\t"

          # attributes in order of useful information (in LFF anyway)
          ### attri_comments
          if(rgdQtlLink.key?(ff[4])) # if optional file is provided 
            f.print "id=#{linkId}; symbolicName=#{ff[4]}"
          else
            f.print "id=#{linkId}"
          end
