...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC delConrad2 table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--delConrad2File', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC delConrad2 table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --delConrad2File       | -r    => UCSC delConrad2 file to convert
    --trackName             | -t    => Track name for delConrad2 track.
                                       (type:subtype)
    --className             | -l    => Class name for delConrad2 track.
  ...
  USAGE:
  BRL_UCSC_delConrad2.rb -r delConrad2.txt.gz -t DEL:Conrad2 -l 'Structural Variants' -i /users/ybai/work/Structural_Variants/converter_delConrad2_new -f delConrad2_LFF.txt -d /users/ybai/work/Structural_Variants/converter_delConrad2_new
"
end
    ...
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        # attri_comments
        f.print "Landmark=#{ff[1]}:#{ff[2]}..#{ff[3]}; thickStart=#{ff[7]}; thickEnd=#{ff[8]}; count1=#{ff[9]}; count2=#{ff[10]}; offspring=#{ff[11]}; population=#{ff[12]}"
        # sequence (none)
        f.print "\t.\t"
