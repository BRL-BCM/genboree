...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC miRNA table to equivalent LFF version
...
PROGRAM DESCRIPTION:
  Converts from UCSC miRNA table to equivalent LFF version.
  ...
  USAGE:
  BRL_UCSC_hg16-miRNA.rb -r miRNA.txt.gz -t RNA -s miRNA -l 'Gene and Gene Prediction' -i /users/ybai/work/Human_hg16-miRNA -f miRNA_LFF.txt -d /users/ybai/work/Human_hg16-miRNA
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    ...
    # Open the file
    reader1 = BRL::Util::TextReader.new("#{cDirectoryInput}/#{miRNAFile}")
    line1 = nil
    ...
    begin
        ...
        reader1.each { |line1|
          ...
          # attri_comments
          f.print "thickStart=#{ff[7]}; thickEnd=#{ff[8]}"
          # sequence (none)
          f.print "\t.\t"
