...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC snp_125 file to equivalent LFF version
# and output 1+ tracks
...
def processArguments()
  optsArray = [
                ...
                ['--snps125File', '-o', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Convert from UCSC snp to equivalent LFF version.  

  COMMAND LINE ARGUMENTS:
    --trackName             | -t    => Track name for Polymorphisms:SNPs (125) track.
                                       (type:subtype)
    --className             | -l    => Class name for Polymorphisms:SNPs (125) track.
    ...
    --snps125File       | -o    => UCSC SNPs (125) file to convert
    --help                  | -h   => [optional flag] Output this usage
                                      info and exit.

  USAGE:
  i.e. BRL_UCSC_snps125.rb -t 'Polymorphisms:SNPs (125)'  -l 'Structural Variants' -i /users/ybai/work/Project4/SNPs_hg17 -f SNPs_125_LFF.txt -d /users/ybai/work/Project4/SNPs_hg17 -o snp125.txt.gz
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    ...
    snps125File = inputsHash['--snps125File'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{snps125File}"))
      $stderr.puts "WARNING: the file '#{snps125File}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    snps125 = Hash.new { |hh, kk| hh[kk] = 0 }

    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{snps125File}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[4].strip!  # name 
        snps125[ff[4]] += 1
        ff[4] = ("#{ff[4]}.#{snps125[ff[4]]}".to_sym) if(snps125[ff[4]] > 1)
