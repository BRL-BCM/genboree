...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Rinn et al. Sex Gene Expression Data table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--rinnSexExpFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Rinn et al. Sex Gene Expression data table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --rinnSexExpFile       | -r    => UCSC Rinn et al. Sex Gene Expression data file to convert
    --trackName             | -t    => Track name for Rinn et al. Sex Gene Expression data track.
                                       (type:subtype)
    --className             | -l    => Class name for Rinn et al. Sex Gene Expression data track.
  ...
  USAGE:
  BRL_UCSC_rinnsexexp.rb -r rinnSex.txt.gz -t 'Special Expression:Sex Gene' -l 'Expression and Regulation' -i /users/ybai/work/Project6/Expression_Regulation/Rinn_Sex_Exp -f rinnSex_LFF.txt -d /users/ybai/work/Project6/Expression_Regulation/Rinn_Sex_Exp
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    rinnSexExpFile = inputsHash['--rinnSexExpFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{rinnSexExpFile}"))
      $stderr.puts "WARNING: the file '#{rinnSexExpFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    rinnSexExp = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{rinnSexExpFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[4].strip!
        rinnSexExp[ff[4]] += 1  
        ff[4] = ("#{ff[4]}.#{rinnSexExp[ff[4]]}".to_sym) if(rinnSexExp[ff[4]] >= 1)
        ...
        #expCount = ff[13].to_i
        #expIds = ff[14].chomp(',').split(/,/).map{|xx| xx.to_i}
        #expScores = ff[15].chomp(',').split(/,/).map{|xx| xx.to_i}
        ...
        #unless(expIds.size == expCount and expScores.size == expCount)
          #$stderr.puts "\n\nERROR: this line doesn't have the right number of blocks (#{expCount}).\n\n#{line}"
        #end
        ...
        blockCount.times { |ii|  ### start from index 0
          ...
          f.print "#{className}\t#{ff[4]}.Block_#{blockNum}\t#{lffType}\t#{lffSubtype}\t#{ff[1]}\t#{ff[2]+chromStarts[ii].to_i+1}\t#{ff[2]+chromStarts[ii].to_i+blockSizes[ii].to_i}\t#{ff[6]}\t.\t#{ff[5]}\t.\t.\t"

          f.print "thickStart=#{ff[7]}; thickEnd=#{ff[8]}; reserved=#{ff[9]}; " +
                "expCount=#{ff[13]}; expIds=#{ff[14]}; expScores=#{ff[15]}; " +
