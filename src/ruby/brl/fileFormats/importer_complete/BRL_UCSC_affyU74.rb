...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Affy U74 table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--affyU74File', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Affy U74 table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --affyU74File       | -r    => UCSC Affy U74 file to convert
    --trackName             | -t    => Track name for Affy U74 track.
                                       (type:subtype)
    --className             | -l    => Class name for Affy U74 track.
  ...
  USAGE:
  BRL_UCSC_affyU74.rb -r affyU74.txt.gz -t AFFY:U74 -l 'Expression and Regulation' -i /users/ybai/work/Project6/Expression_Regulation/Affy_U74 -f affyU74_LFF.txt -d /users/ybai/work/Project6/Expression_Regulation/Affy_U74
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    affyU74File = inputsHash['--affyU74File'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{affyU74File}"))
      $stderr.puts "WARNING: the file '#{affyU74File}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    affyU74 = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{affyU74File}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        affyU74[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{affyU74[ff[10]]}".to_sym) if(affyU74[ff[10]] >= 1)
