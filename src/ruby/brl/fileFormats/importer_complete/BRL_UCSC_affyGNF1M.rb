...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Affy GNF1M table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--affyGNF1MFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Affy GNF1M table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --affyGNF1MFile       | -r    => UCSC Affy GNF1M file to convert
    --trackName             | -t    => Track name for Affy GNF1M track.
                                       (type:subtype)
    --className             | -l    => Class name for Affy GNF1M track.
  ...
  USAGE:
  BRL_UCSC_affyGNF1M.rb -r affyGnf1m.txt.gz -t AFFY:GNF1M -l 'Expression and Regulation' -i /users/ybai/work/Project6/Expression_Regulation/Affy_GNF1M -f affyGnf1m_LFF.txt -d /users/ybai/work/Project6/Expression_Regulation/Affy_GNF1M
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    affyGNF1MFile = inputsHash['--affyGNF1MFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{affyGNF1MFile}"))
      $stderr.puts "WARNING: the file '#{affyGNF1MFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    affyGNF1M = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{affyGNF1MFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        affyGNF1M[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{affyGNF1M[ff[10]]}".to_sym) if(affyGNF1M[ff[10]] >= 1)
