...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Affy U74C table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--affyU74CFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Affy U74C table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --affyU74CFile       | -r    => UCSC Affy U74C file to convert
    --trackName             | -t    => Track name for Affy U74C track.
                                       (type:subtype)
    --className             | -l    => Class name for Affy U74C track.
  ...
  USAGE:
  BRL_UCSC_affyU74C.rb -r affyGnfU74C.txt.gz -t AFFY:U74C -l 'Expression and Regulation' -i /users/ybai/work/Project6/Expression_Regulation/Affy_U74C -f affyGnfU74C_LFF.txt -d /users/ybai/work/Project6/Expression_Regulation/Affy_U74C
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    affyU74CFile = inputsHash['--affyU74CFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{affyU74CFile}"))
      $stderr.puts "WARNING: the file '#{affyU74CFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    affyU74C = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{affyU74CFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[4].strip!  # name 
        affyU74C[ff[4]] += 1  
        ff[4] = ("#{ff[4]}.#{affyU74C[ff[4]]}".to_sym) if(affyU74C[ff[4]] >= 1)
