...
require 'net/ftp'
require 'brl/fileFormats/trackImporter/trackImporter.ucscConverter'
...
module BRL; module TrackImporter
    class UcscConverterEngine
      ...
      def convertFile()
        ...
        begin
          converterClass = UCSCConverter.new()
      ...
      #  variables
      # [+returns+]   nothing
      def UcscConverterEngine.setParameters(oldHash)
      ...
      # ones are met.
      # [+returns+]   nothing
      def UcscConverterEngine.processArguments()
        ...
        UCSCConverterEngine.usage() if(optsHash.key?('--help'));

        unless(progOpts.getMissingOptions().empty?)
          UCSCConverterEngine.usage("USAGE ERROR: some required arguments are missing")
        end

        UCSCConverterEngine.usage() if(optsHash.empty?);

        optsHash = UCSCConverterEngine.setParameters(optsHash);
      ...
      # [+returns+]   nothing
      def UcscConverterEngine.usage(msg='')
        unless(msg.empty?)
          puts("\n#{msg}\n")
        end
        ...
        USAGE:
        trackImporter.ucscConverterEngine.rb -k key -c Class -t Type -s SubType -o config.json -i http://someURL.edu/all_bacends.sql -d http://someURL.edu/all_bacends.txt.gz -f all_bacends.out.data -u 12345 -r 987654";
        exit(134);
      end
    end
  end; end
