...
require 'brl/fileFormats/trackImporter/trackImporter.ensemblGenes2lff'
# == Overview.
...
module BRL; module TrackImporter
    class EnsembleGeneEngine
      # CONSTRUCTOR. Creates an instance of the EnsembleGeneEngine class and
      ...
      def convertFile()
        ...
        begin
          converterClass = EnsembleGene2lff.new()

          optsHash = Hash.new()
          optsHash['--nameOfOutputFile'] = "#{@trackData.outputFile}"
          optsHash['--outputClass'] ="#{@trackData.lffClass}"
          optsHash['--outputType'] ="#{@trackData.lffType}"
          optsHash['--outputSubtype'] ="#{@trackData.lffSubType}"
          optsHash['--localDirectory'] = "#{Dir.pwd}"
          optsHash['--ensGeneFileName'] = "#{@trackData.dataFile.split(",")[0]}"
          optsHash['--ccdsInfoFileName'] = "#{@trackData.dataFile.split(",")[1]}"
          optsHash['--ensGtpFileName'] = "#{@trackData.dataFile.split(",")[2]}"
          optsHash['--sfDescriptionFileName'] = "#{@trackData.dataFile.split(",")[3]}"
          optsHash['--refFlatFileName'] = "#{@trackData.dataFile.split(",")[4]}"
          optsHash['--knownToEnsemblFileName'] =  "#{@trackData.dataFile.split(",")[5]}"
          optsHash['--ensPepFileName'] =  "#{@trackData.dataFile.split(",")[6]}"
          optsHash['--knownToLocusLinkFileName'] = "#{@trackData.dataFile.split(",")[7]}"
          optsHash['--knownToRefSeqFileName'] = "#{@trackData.dataFile.split(",")[8]}"
          optsHash['--doNotFtp'] = "true"
      ...
      #  variables
      # [+returns+]   nothing
      def EnsembleGeneEngine.setParameters(oldHash)
      ...
      # ones are met.
      # [+returns+]   nothing
      def EnsembleGeneEngine.processArguments()
        ...
        EnsembleGeneEngine.usage() if(optsHash.key?('--help'));

        unless(progOpts.getMissingOptions().empty?)
          EnsembleGeneEngine.usage("USAGE ERROR: some required arguments are missing")
        end

        EnsembleGeneEngine.usage() if(optsHash.empty?);

        optsHash = EnsembleGeneEngine.setParameters(optsHash);
      ...
      # [+returns+]   nothing
      def EnsembleGeneEngine.usage(msg='')
        unless(msg.empty?)
          puts("\n#{msg}\n")
        end
        ...
        USAGE:
        trackImporter.ensembleGeneEngine.rb -k key -c Class -t Type -s SubType -o config.json -i http://someURL.edu/all_bacends.sql -d http://someURL.edu/all_bacends.txt.gz -f all_bacends.out.data -u 12345 -r 987654";
        exit(134);
      end
    end
  end; end
