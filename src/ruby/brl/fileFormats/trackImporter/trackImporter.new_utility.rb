...
module BRL; module TrackImporter
    class Utility
      ...
      def Utility.uploadFile(userId, refSeqId, outputFile)
        grp = "arpit_group"
        db ="test"
        host = "proline.brl.bcmd.bcm.edu"
        path = "/REST/v1/grp/{grp}/db/{db}/"
        resource = path +"/annos?format=lff&userId=arpit"
        apiCaller = ApiCaller.new(host,resource,"arpit","sonali")
        infile = File.open(outputFile,"r")
        httpResp = apiCaller.puts(infile)
