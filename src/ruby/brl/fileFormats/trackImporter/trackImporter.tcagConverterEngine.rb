...
require 'net/ftp'
require 'brl/fileFormats/trackImporter/trackImporter.tcagConverter'
...
module BRL; module TrackImporter
    class TcagConverterEngine
      # CONSTRUCTOR. Creates an instance of the UCSCConverterEngine class and
      ...
      def convertFile()
        ...
        @trackData.configFile = "#{@genbConf.resourcesDir}importer/#{@trackData.configFile}"

        begin
          converterClass = TCAGConverter.new()
          converterClass.run(@trackData)
      ...
      #  variables
      # [+returns+]   nothing
      def TcagConverterEngine.setParameters(oldHash)
      ...
      # ones are met.
      # [+returns+]   nothing
      def TcagConverterEngine.processArguments()
        ...
        TCAGConverterEngine.usage() if(optsHash.key?('--help'));

        unless(progOpts.getMissingOptions().empty?)
          TCAGConverterEngine.usage("USAGE ERROR: some required arguments are missing")
        end

        TCAGConverterEngine.usage() if(optsHash.empty?);

        optsHash = TCAGConverterEngine.setParameters(optsHash);
      ...
      # [+returns+]   nothing
      def TcagConverterEngine.usage(msg='')
        unless(msg.empty?)
          puts("\n#{msg}\n")
        end
        puts "PROGRAM DESCRIPTION:
        The TCAG To LFF converter Engine serves as a way of converting information from
        the TCAG database to the Genboree LFF format.
        ...
        USAGE:
        trackImporter.tcagConverterEngine.rb -k key -c Class -t Type -s SubType -o config.json -i http://someURL.edu/all_bacends.sql -d http://someURL.edu/all_bacends.txt.gz -f all_bacends.out.data -u 12345 -r 987654";
        exit(134);
      end
    end
  end; end
