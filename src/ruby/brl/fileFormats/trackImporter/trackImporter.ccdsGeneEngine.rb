...
require 'brl/fileFormats/trackImporter/trackImporter.ccdsGenes2lff'
# == Overview.
...
module BRL; module TrackImporter
    class CcdsGeneEngine
      # CONSTRUCTOR. Creates an instance of the CcdsGeneEngine class and
      ...
      def convertFile()
        ...
        begin
          converterClass = CCDSGene2lff.new()
          ...
          optsHash['--ccdsGeneFile'] = "#{@trackData.dataFile.split(",")[0]}"
          optsHash['--ccdsKgMapFile'] = "#{@trackData.dataFile.split(",")[1]}"
          optsHash['--ccdsInfoFile'] = "#{@trackData.dataFile.split(",")[2]}"
          optsHash['--knownToEnsemblFile'] = "#{@trackData.dataFile.split(",")[3]}"
          optsHash['--knownToRefSeqFile'] = "#{@trackData.dataFile.split(",")[4]}"
          optsHash['--knownGeneFile'] = "#{@trackData.dataFile.split(",")[5]}"
          optsHash['--knownToLocusLinkFile'] = "#{@trackData.dataFile.split(",")[6]}"
          optsHash['--refFlatFile'] = "#{@trackData.dataFile.split(",")[7]}"
      ...
      #  variables
      # [+returns+]   nothing
      def CcdsGeneEngine.setParameters(oldHash)
      ...
      # ones are met.
      # [+returns+]   nothing
      def CcdsGeneEngine.processArguments()
        ...
        CcdsGeneEngine.usage() if(optsHash.key?('--help'));

        unless(progOpts.getMissingOptions().empty?)
          CcdsGeneEngine.usage("USAGE ERROR: some required arguments are missing")
        end

        CcdsGeneEngine.usage() if(optsHash.empty?);

        optsHash = CcdsGeneEngine.setParameters(optsHash);
      ...
      def CcdsGeneEngine.usage(msg='')
        ...
        USAGE:
        trackImporter.ccdsGeneEngine.rb -k key -c Class -t Type -s SubType -o config.json -i http://someURL.edu/all_bacends.sql -d http://someURL.edu/all_bacends.txt.gz -f all_bacends.out.data -u 12345 -r 987654";
        exit(134);
      end
    end
  end; end
