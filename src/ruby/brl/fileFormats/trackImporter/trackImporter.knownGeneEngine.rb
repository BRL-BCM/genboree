...
require 'brl/fileFormats/trackImporter/trackImporter.knownGene2lff'
...
module BRL; module TrackImporter
    class KnownGeneEngine
      # CONSTRUCTOR. Creates an instance of the KnownGeneEngine class and
      ...
      def convertFile()
        ...
        begin
          converterClass = KnownGene2lff.new()
                    
          optsHash = Hash.new()
          optsHash['--knownGeneFile'] = "#{@trackData.dataFile.split(",")[0]}"
          optsHash['--trackName'] =  "#{@trackData.lffType}:#{@trackData.lffSubType}"
          optsHash['--kgAlias'] = "#{@trackData.dataFile.split(",")[1]}"
          optsHash['--keggPathway'] = "#{@trackData.dataFile.split(",")[2]}"
          optsHash['--kgXref'] = "#{@trackData.dataFile.split(",")[3]}"
          optsHash['--ensembl'] = "#{@trackData.dataFile.split(",")[4]}"
          optsHash['--locusLink'] = "#{@trackData.dataFile.split(",")[5]}"
          optsHash['--kg3tokg4'] = "#{@trackData.dataFile.split(",")[6]}"
          optsHash['--kg2tokg3'] = "#{@trackData.dataFile.split(",")[7]}"
      ...
      #  variables
      # [+returns+]   nothing
      def KnownGeneEngine.setParameters(oldHash)
      ...
      # ones are met.
      # [+returns+]   nothing
      def KnownGeneEngine.processArguments()
        ...
        KnownGeneEngine.usage() if(optsHash.key?('--help'));

        unless(progOpts.getMissingOptions().empty?)
          KnownGeneEngine.usage("USAGE ERROR: some required arguments are missing")
        end

        KnownGeneEngine.usage() if(optsHash.empty?);

        optsHash = KnownGeneEngine.setParameters(optsHash);
      ...
      def KnownGeneEngine.usage(msg='')
        ...
        USAGE:
        trackImporter.knownGeneEngine.rb -k key -c Class -t Type -s SubType -o config.json -i http://someURL.edu/all_bacends.sql -d http://someURL.edu/all_bacends.txt.gz -f all_bacends.out.data -u 12345 -r 987654";
        exit(134);
      end
    end
  end; end
