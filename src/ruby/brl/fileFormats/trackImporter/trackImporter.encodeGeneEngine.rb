...
require 'brl/fileFormats/trackImporter/trackImporter.encodeGene2lff'
...
module BRL; module TrackImporter
    class EncodeGeneEngine
      # CONSTRUCTOR. Creates an instance of the EncodeGeneEngine class and
      ...
      def convertFile()
        ...
        begin
          converterClass = EncodeGene2lff.new()

          optsHash = Hash.new()
          dataFiles = @trackData.dataFile.split(",")
          optsHash['--trackName'] =  "#{@trackData.lffType}:#{@trackData.lffSubType}"
          optsHash['--refFlatFile'] = dataFiles[0] if(dataFiles[0])
          optsHash['--refGeneFile'] = dataFiles[1] if(dataFiles[1])
          optsHash['--refLinkFile'] = dataFiles[2] if(dataFiles[2])
          optsHash['--refSeqStatusFile'] = dataFiles[3] if(dataFiles[3])
          optsHash['--refSeqSummaryFile'] = dataFiles[4] if(dataFiles[4])
      ...
      #  variables
      # [+returns+]   nothing
      def EncodeGeneEngine.setParameters(oldHash)
      ...
      # ones are met.
      # [+returns+]   nothing
      def EncodeGeneEngine.processArguments()
        ...
        EncodeGeneEngine.usage() if(optsHash.key?('--help'));

        unless(progOpts.getMissingOptions().empty?)
          EncodeGeneEngine.usage("USAGE ERROR: some required arguments are missing")
        end

        EncodeGeneEngine.usage() if(optsHash.empty?);

        optsHash = EncodeGeneEngine.setParameters(optsHash);
      ...
      # [+returns+]   nothing
      def EncodeGeneEngine.usage(msg='')
        unless(msg.empty?)
          puts("\n#{msg}\n")
        end
        ...
        USAGE:
        trackImporter.encodeGeneEngine.rb -k key -c Class -t Type -s SubType -o config.json -i http://someURL.edu/all_bacends.sql -d http://someURL.edu/all_bacends.txt.gz -f all_bacends.out.data -u 12345 -r 987654";
        exit(134);
      end
    end
  end; end
