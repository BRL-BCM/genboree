...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Affy MOE430 table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--affyMOE430File', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC Affy MOE430 table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --affyMOE430File       | -r    => UCSC Affy MOE430 file to convert
    --trackName             | -t    => Track name for Affy MOE430 track.
                                       (type:subtype)
    --className             | -l    => Class name for Affy MOE430 track.
  ...
  USAGE:
  BRL_UCSC_affyMOE430.rb -r affyMOE430.txt.gz -t AFFY:MOE430 -l 'Expression and Regulation' -i /users/ybai/work/Project6/Expression_Regulation/Affy_MOE430 -f affyMOE430_LFF.txt -d /users/ybai/work/Project6/Expression_Regulation/Affy_MOE430
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    affyMOE430File = inputsHash['--affyMOE430File'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{affyMOE430File}"))
      $stderr.puts "WARNING: the file '#{affyMOE430File}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    affyMOE430 = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{affyMOE430File}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        affyMOE430[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{affyMOE430[ff[10]]}".to_sym) if(affyMOE430[ff[10]] >= 1)
