...
begin
  ...
  optsHash = EnsemblGenes2lff.processArguments(outs)
  ...
  $stderr.puts "#{Time.now()} Finishing the process" if(outs[:verbose])
      ...
      errTitle =  "#{Time.now()} ensemblGenes2lff - FATAL ERROR: The liftover exited without processing all the data,
      ...
      $stderr.puts msgTitle unless(!outs[:optsHash].nil? or outs[:optsHash].key?('--help'))
