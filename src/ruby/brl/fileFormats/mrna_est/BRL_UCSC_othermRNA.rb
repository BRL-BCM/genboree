...
# PURPOSE
# ##############################################################################
# Simple: convert from UCSC Other mRNA table to equivalent LFF version
...
def processArguments()
  optsArray = [
                ['--othermRNAFile', '-r', GetoptLong::REQUIRED_ARGUMENT],
...
PROGRAM DESCRIPTION:
  Converts from UCSC other mRNA table to equivalent LFF version.

  COMMAND LINE ARGUMENTS:
    --othermRNAFile       | -r    => UCSC other mRNA file to convert
    --trackName             | -t    => Track name for other mRNA track.
                                       (type:subtype)
    --className             | -l    => Class name for other mRNA track.
  ...
  USAGE:
  BRL_UCSC_othermRNA.rb -r xenoMrna.txt.gz -t UCSC:xenomRNA -l 'mRNA and EST' -i /users/ybai/work/Human_Project6/mRNA_EST/Other_mRNA -f xenomRNA_LFF.txt -d /users/ybai/work/Human_Project6/mRNA_EST/Other_mRNA
"
end
...
class MyConverter
  ...
  def convert(inputsHash) 
    othermRNAFile = inputsHash['--othermRNAFile'].strip
    ...
    unless(File.size?("#{cDirectoryInput}/#{othermRNAFile}"))
      $stderr.puts "WARNING: the file '#{othermRNAFile}' is empty. Nothing to do."
      exit(FAILED)
    end
    ...
    othermRNA = Hash.new { |hh, kk| hh[kk] = 0 }
    # Open the file
    reader = BRL::Util::TextReader.new("#{cDirectoryInput}/#{othermRNAFile}")
    line = nil
    begin
     ...
     # Go through each line
      reader.each { |line|
        ...
        ff[10].strip!
        othermRNA[ff[10]] += 1  
        ff[10] = ("#{ff[10]}.#{othermRNA[ff[10]]}".to_sym) if(othermRNA[ff[10]] >= 1)
        ...
        blockCount.times { |ii|  ### start from index 0
          ...
          f.print "#{className}\t#{ff[10]}.Block_#{blockNum}\t#{lffType}\t#{lffSubtype}\t#{ff[14]}\t#{tStarts[ii].to_i+1}\t#{tStarts[ii].to_i+blockSizes[ii].to_i}\t#{abc}\t.\t1.0\t.\t.\t"
