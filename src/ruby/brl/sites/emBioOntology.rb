...
module BRL; module Sites
  ...
  class EMBioOntology < AbstractSite
    ...
    DEFAULT_API_KEY = "79d29552-b8c1-4088-a7d0-4f89dd21b810"
    ...
    attr_accessor :debug, :lastError, :response, :collection, :callbackObj
    ...
    def callProcessCacheHeaders(updateStatus, falsePosStatus, opts, url, callbkMethod)
      idx = opts[:idx]
      retVal = {}
      $stderr.debugPuts(__FILE__, __method__, "DEBUG", "Should we update cache or not??") if(@debug)
      ...
      if(updateStatus and falsePosStatus)
        ...
        $stderr.debugPuts(__FILE__, __method__, "DEBUG", "Refusing to update cache because request to #{HOST.inspect} has shown false positives") if(@debug)
      elsif(updateStatus and !falsePosStatus)
        # then we want to update and we are ok to do so
        $stderr.debugPuts(__FILE__, __method__, "DEBUG", "Updating cache because existing cache entry has expired and test request to #{HOST.inspect} has not shown false positives") if(@debug)
    ...
    def emRequest(url, headers={}, opts={}, callbkMethod=:processCacheHeadersDefaultCallbk)
      ...
      http.callback {
        ...
        tt3 = Time.now
        requestTimeDatum[:"read response"] = tt3 - tt2
        #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "in emRequest; content:\n\n#{content.inspect}")
        begin
          parsedResp = JSON.parse(content)
        rescue ::JSON::ParserError => err
          parsedResp = nil
    ...
    def mapAdditionalPagesToSameHeadersUsedByFirstPage(opts)
      ...
      pageLinks.each { |pageLink| 
        ...
        emRequest(pageLink, headers, opts, :pageLinksIteratorCallBack)
      }
      if(pageLinks.size == 0)
        #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "@link2Resp:\n\n#{@link2Resp.inspect}")
        @link2Resp.each_key{|link|
          parsedResp = @link2Resp[link]
          @collection += parsedResp[COLLECTION_KEY]
        }
        if(@callbackObj)
