...
module BRL; module Sites
  ...
  class EMBioOntology < AbstractSite
    ...
    attr_accessor :debug, :lastError, :response, :collection, :callbackObj, :prefLabelForTerm
    ...
    def numThreads()
      ...
      return ( @doingMemoization ? 3 : 1 ) # May be able to do more memoized calls if only 1 thread, but tests don't look good for that solving things
    end
    ...
    def requestTermsByNameViaSubtree(term, prefix=false, maxSize=nil, opts={})
      ...
      begin
        ...
        tt1 = Time.now
        collectAndSearchForTerm(term, opts)
      rescue => err
        retVal = nil
        logError(err)
      end
    ...
    def collectAndSearchForTerm(term, opts={})
      ...
      @ontologies.each_index { |ii|
        ...
        # Gets cache headers. checks if the cache is too old or not.
        #  -- If it's deemed fresh, will use cache otherwise will query bioportal and update cache if results seem valid
        getCacheHeaders(url, ii, :processCacheHeadersDefaultCallbk, opts)
      }
    end
    ...
    def searchCollectionForTerm(collection, term, opts={})
      ...
      #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "matches:\n\n#{matches.inspect}\n\nopts: #{opts.inspect}\n\ncollection:\n\n#{collection.inspect}")
      return matches
    end
    ...
    def parallelSearches(urls, opts={})
      ...
      $stderr.debugPuts(__FILE__, __method__, 'DEBUG-WTH', "About to do Parallel.map via threads...aren't we supposed to be using EM & callbacks??")
      collections = ::Parallel.map(urls, :in_threads => opts[:threads]) { |url|
        begin
          $stderr.debugPuts(__FILE__, __method__, 'DEBUG-WTH', "Why does the following not fail? Missing args. Unported methods assumed not to be used or getting lucky and this particular used path is never exercised (because everything alwasy goes as expected")
          headers = getCacheHeaders(url) # getCacheHeaders(url, idx, callbkMethod, opts={})
    ...
    def getCacheHeaders(url, idx, callbkMethod, opts={})
      # check if we should update cache (or if we are using a proxy cache at all)
      updateCache?(url, idx, callbkMethod, opts)
    end

    # Check if cache entry is too old
    def updateCache?(url, idx, callbkMethod, opts={})
      proxy = true 
      #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "@proxyPath" )
      ...
      if(proxy)
        ...
        uriObj = URI.parse(url)
        http = EventMachine::HttpRequest.new(url, :connect_timeout => 10, :inactivity_timeout => 30).head
        http.callback {
          updateStatus = false
          http.response_header.each_key { |header|
            http.response_header[header] = [http.response_header[header]]
          }
          #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "\n\nhttp.response.header:\n\n#{http.response_header.inspect}" )
          respHeaders = http.response_header
          ...
          updateCacheCbk(updateStatus, url, idx, callbkMethod, opts)
        }
        http.errback { $stderr.debugPuts(__FILE__, __method__, "ERROR", "url: #{url}\n\n#{http.response.inspect}" ) }
      else
        updateCacheCbk(false, url, idx, callbkMethod, opts)  
      end
    end
    
    def updateCacheCbk(updateStatus, url, idx, callbkMethod, opts={})
      retVal = {}
      ...
      if(updateStatus)
        ...
        opts[:idx] = idx
        opts[:callbkMethod] = callbkMethod
        emRequest(url, headers, opts, :queryBioPortalForUpdatingCache)
      else
        opts[:idx] = idx
        callProcessCacheHeaders(updateStatus, falsePosStatus, opts, url, callbkMethod)
      end
    end
    
    def queryBioPortalForUpdatingCache(parsedResp, opts, headers, url)
      idx = opts[:idx]
      updateStatus = true
      falsePosStatus = false
      unless(parsedResp.nil?)
      ...
      callbkMethod = opts[:callbkMethod]
      callProcessCacheHeaders(updateStatus, falsePosStatus, opts, url, callbkMethod)
    end
    
    def callProcessCacheHeaders(updateStatus, falsePosStatus, opts, url, callbkMethod)
      idx = opts[:idx]
      retVal = {}
      $stderr.debugPuts(__FILE__, __method__, "DEBUG", "Should we update proxy cache or not??") if(@debug)
      ...
      if(updateStatus and falsePosStatus)
        ...
        $stderr.debugPuts(__FILE__, __method__, "DEBUG", "Refusing to update proxy cache because request to #{HOST.inspect} has shown false positives") if(@debug)
      elsif(updateStatus and !falsePosStatus)
        # then we want to update and we are ok to do so
        $stderr.debugPuts(__FILE__, __method__, "DEBUG", "Updating cache because existing cache entry has expired and test request to #{HOST.inspect} has not shown false positives. Update record for this url:\n\t#{url.inspect}\n\n") if(@debug)
        retVal[PROXY_BYPASS_CACHE] = true.to_s
      else
        $stderr.debugPuts(__FILE__, __method__, 'DEBUG', "Not updating because existing cache entry not expired based on its header date.")
      end
      processCacheHeaders(retVal, url, idx, callbkMethod, opts)
    end
    
    def emRequest(url, headers={}, opts={}, callbkMethod=:processCacheHeadersDefaultCallbk)
      #$stderr.debugPuts(__FILE__, __method__, "BIOONTOLOGY", "making request at url=#{url.inspect}")
      requestTimeDatum = {}
      parsedResp = nil
      tt1 = tt2 = tt3 = tt4 = nil
      ...
      opts = supportedOpts.merge(opts)
      tt1 = Time.now
      # A begin rescue is required when we instantiate the em http client since initializing it with a nil object can cause EventMachine to halt bringing down thin in the process. 
      #  - An empty response is much better than thin crashing!!
      begin
        uriObj = URI.parse(url)
        http = EventMachine::HttpRequest.new(url, :connect_timeout => 10, :inactivity_timeout => 60).get :head => headers
      rescue => err
      ...
      http.callback {
        begin
          ...
          if(@errors[url] and @proxyHost and @proxyPort and online)
            ...
            opts[:depth] = depth
            unless(opts[:depth] > 1)
              emRequest(url, headers, opts, callbkMethod)
            end
          end
          requestTimeDatum[:"error checking"] = Time.now - tt4
          requestTimeDatum[:full] = Time.now - tt1
          @requestTimeData << requestTimeDatum if(@debug)
          self.send(callbkMethod, parsedResp, opts, headers, url)
      ...
      http.errback {
        begin
          parsedResp = nil
          $stderr.debugPuts(__FILE__, __method__, "FAILED", "url: #{url}. Connection closed or timeout encountered" )
          @errors[url] = "Connection closed or timeout encountered"
          self.send(callbkMethod, parsedResp, opts, headers, url)
        rescue Exception => err
          logError(err)
          @callbackObj.prepareFinalResp(nil)
        end
      }
      
    end
    
    def processCacheHeaders(headers, url, idx, callbkMethod, opts={})
      emRequest(url, headers, opts, callbkMethod)
    end
    
    def processCacheHeadersDefaultCallbk(parsedResp, opts, headers, url)
      @noOfCallbksDone += 1
      idx = opts[:idx]
      ...
      while(respIsError and jj < PRIME_PAGE_SIZES.length)
        ...
        $stderr.debugPuts(__FILE__, __method__, "DEBUG", "resp is error and jj is smaller tham PRIME_PAGE_SIZES") 
      ...
      if(pageLinks.nil? or respIsError)
        if(respIsError)
          $stderr.debugPuts(__FILE__, __method__, "BIOONTOLOGY", "unable to get first page results for the final url: #{url.inspect}")
        end
      ...
      if(@noOfCallbksDone == @ontologies.size)
        processSubTreeRoots(opts)
      end
    end

    def processSubTreeRoots(opts)
      unless(@subtrees.nil? or @subtrees.empty?)
        @noOfCallbksDone = 0
        @subtrees.each_index { |idx|
          subtree = @subtrees[idx]
          ontology = @ontologies[idx]
          unless(subtree.nil? or subtree.empty?)
            ...
            opts[:path] = path
            getCacheHeaders(url, idx, :processSubTreeRootCallbk, opts) 
          end
        }
      else
        $stderr.debugPuts(__FILE__, __method__, 'DEBUG-WTH', "Why does this not fail? Missing opts. Lucky assumption that there are always subtrees is never violated? Dangerous. Perhaps default opts={} would be best for this method declaration")
        mapAdditionalPagesToSameHeadersUsedByFirstPage()
      end
    end
    
    def processSubTreeRootCallbk(parsedResp, opts, headers, url)
      idx = opts[:idx]
      @noOfCallbksDone += 1
      #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "in processSubTreeRootCallbk")
      if(!parsedResp.nil?)
        #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "parsedResp:\n\n#{parsedResp.inspect}")
        if(@term.nil?)
        ...
        mapAdditionalPagesToSameHeadersUsedByFirstPage(opts) if(@noOfCallbksDone == @subtrees.size)
      else
        ...
        newUrl = buildUrl(HOST, opts[:path], newQuery)
        emRequest(newUrl, headers, { :idx => idx }, :callMapAdditionalPagesToSameHeadersUsedByFirstPage) 
      end
    end
    
    def callMapAdditionalPagesToSameHeadersUsedByFirstPage(parsedResp, opts, headers, url)
      idx = opts[:idx]
      if(!parsedResp.nil?)
        if(@term.nil?)
          @collection.push(parsedResp)
    ...
    def mapAdditionalPagesToSameHeadersUsedByFirstPage(opts)
      pageUrl2Headers = {}
      ...
      @pageLinksSize = pageLinks.size
      @pageLinksIterator = 0
      pageLinks.each { |pageLink| 
        headers = pageUrl2Headers[pageLink]
        headers = {} if headers.nil?
        #parsedResp = requestWrapper(pageLink, headers, opts) rescue nil
        opts[:pageLink] = pageLink
        emRequest(pageLink, headers, opts, :pageLinksCacheHeadersCallBack)
      }
      if(pageLinks.size == 0)
        #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "@link2Resp:\n\n#{@link2Resp.inspect}")
        @link2Resp.each_key{|link|
          parsedResp = @link2Resp[link]
          @collection += parsedResp[COLLECTION_KEY] if(!parsedResp.nil? and !parsedResp.empty? and !parsedResp.is_a?(Array) )
        }
        if(@callbackObj)
          if(@callbackObj.respond_to?(:prepareFinalResp))
            @callbackObj.prepareFinalResp(@collection)
          end
        end
        #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "@collection: #{@collection.inspect}")
        $stderr.debugPuts(__FILE__, __method__, "DEBUG", "errors from #{HOST}:\n\n#{JSON.pretty_generate(@errors)}") if(@debug and !@errors.empty?)
      end
    end

    def pageLinksCacheHeadersCallBack( parsedResp, opts, headers, url)
      #$stderr.debugPuts(__FILE__, __method__, 'DEBUG NON-1ST PAGE', "Check page link headers to see if need to update cache record. Need to get and process the REAL page headers since 'headers' argument is first-page's headers. If we don't, will have a cache bug where non-first pages are not updated. url arg is:\n\t#{url.inspect}\n\n")
      # After dealing with cache update issues, need to resume with pageLinksIteratorCallBack().
      getCacheHeaders(url, nil, :pageLinksIteratorCallBack, opts)
    end

    def pageLinksIteratorCallBack(parsedResp, opts, headers, url)
      #$stderr.debugPuts(__FILE__, __method__, 'DEBUG NON-1ST PAGE', "Entered with url of:\n\t #{url.inspect}")
      @pageLinksIterator += 1
      respIsError = errorResp?(parsedResp)
      pageLink = opts[:pageLink]
      if(respIsError)
      ...
      @link2Resp[pageLink] = parsedResp
      if(@pageLinksIterator == @pageLinksSize)
        addToCollectionFromLink2Resp()
      end
    end
    
    def addToCollectionFromLink2Resp()
      #$stderr.debugPuts(__FILE__, __method__, "DEBUG", "@link2Resp:\n\n#{@link2Resp.inspect}")
      @link2Resp.each_key{|link|
        parsedResp = @link2Resp[link]
