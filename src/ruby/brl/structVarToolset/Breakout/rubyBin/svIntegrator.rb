  ...
  def setParameters()
		...
		@targetSVLFFFilesList.each {|targetName|
			...
			r.each {|l|
				...
				l =~/svIntegratorOverlap=([^;]+)(;|\s|$)/
                                next if (! @querySVHash.key?(svName))
	...
	def intersectWithGenomicFeatures()
		...
		@targetGenomicFilesList.each {|featureName|
			...
			svFeatureCommand = "svLffTrackCoverage.rb -s #{@querySVLff} -c #{featureName} -r #{@radiusFeatures} -S #{tmpIntFile} -C #{tmpIntFile}.xls "
