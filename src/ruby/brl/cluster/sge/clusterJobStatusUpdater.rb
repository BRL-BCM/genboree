...
class ClusterJobStatusUpdater
  ...
  def work()
    ...
    updateHash["sgeJobID"] = @cjobId unless @cjobId.nil?
    updateHash["sgeHostname"] = @hostname unless @hostname.nil?
    updateHash["sgeSubmissionDate"] = Time.parse(@submissionTime) unless @submissionTime.nil?
    updateHash["sgeStartDate"] = Time.parse(@beginTime) unless @beginTime.nil?
    updateHash["sgeEndDate"] = Time.parse(@endTime) unless @endTime.nil?
    updateHash.each_key{ |key| puts key.to_s+"=>"+updateHash[key].to_s }
