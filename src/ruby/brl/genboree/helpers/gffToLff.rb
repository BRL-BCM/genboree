...
class GffToLff
      ...
      def work()
	    ...
	    if (@optsHash.key?('--outputFile'))
		  ...
		  if( inputFileName =~/gff3/)
			fileType = 1
			outputFileName = @optsHash["--inputFile"].sub(".gff3",".lff")
		  else
			...
			if( line =~ /\n$/ )
			   ...
			   unless( line  =~ /##/ or line =~ /track/)
				  ...
				  inc = 0
				    ...
				    writeBuffer << "#{clas}\t#{name}\t#{type}\t#{subType}\t#{entryPoint}\t#{start}\t#{stop}\t#{strand}\t#{phase}\t#{score}\t" + 
					     "#{qStart}\t#{qStop}\tattribute=#{column[8]}\n"
                              end
                           end
			else
			      orphan = line
