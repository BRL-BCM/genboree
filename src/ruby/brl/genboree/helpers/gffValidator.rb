...
module BRL; module FileFormats
  ...
  class GffValidator
    ...
    def validateRefSeqArray(fields)
        ...
        reader.each { |line|
         ...
         if ( @currLineNum > 3)
          ...
          if(ff.size == 9 )
            ...
            valid = GffValidator.validateMain(ff, format, @eps)
          else
    ...
    def GffValidator.validateMain(fields, format,  validEPHash = nil)
      ...
      if(checkValidEP)
          if(!validEPHash.key?(fields[0]))
            error.message += "- referring to unknown entry point '#{fields[0]}'\n"
            retVal = error
          else # found correct refseq, but is coords ok?
            if(lffStop > validEPHash[fields[3]])
              error.message += ("- end of annotation (5th column: '#{fields[4]}') is either \n" +
                                "  not an integer or beyond end of entry point\n" +
                                "  (length of '#{fields[3]}' is #{validEPHash[fields[4]]})\n")
              retVal = error
            end
            if(lffStart > validEPHash[fields[3]])
              error.message += ("- start of annotation (4th column: '#{fields[3]}') is either \n" +
                                "  not an integer or beyond end of entry point\n" +
                                "  (length of '#{fields[3]}' is #{validEPHash[fields[3]]})\n")
              retVal = error
            end
          end
        end
      if(fields.size != 9)
        error.message += "- Not an Gff [entry point] record. It only has #{fields.size} but should have 9\n"
        retVal = error
      end
