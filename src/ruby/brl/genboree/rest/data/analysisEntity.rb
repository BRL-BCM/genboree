...
module BRL ; module Genboree ; module REST ; module Data
  ...
  class AnalysisEntity < BRL::Genboree::REST::Data::AbstractEntity
    ...
    SIMPLE_FIELD_NAMES = [ "name", "type", "dataLevel", "experiment", "state", "avpHash" ]
    ...
    # columns.
    TABBED_HEADER = [ "name", "type", "dataLevel", "experiment", "state" ]
    # Name of this Analysis
    attr_accessor :name
    # +Type+
    attr_accessor :type
    # +Lab+
    attr_accessor :dataLevel
    # +Contributors+
    attr_accessor :experiment
    # +State+
    attr_accessor :state
    # +avpHash+ All of the attribute value pairs for this Analysis, in a Hash
    ...
    # [+dataLevel+] [optional; default=-1]
    # [+experiment+] [optional; default=""]
    # [+state+] [optional; default=0]
    def initialize(doRefs=true, name="", type="", dataLevel=-1, experiment="", state=0, avpHash=Hash.new)
      super(doRefs)
      self.update(name, type, dataLevel, experiment, state, avpHash)
    end

    # REUSE INSTANCE. Update this instance with new data; supports reuse of
    #   instances rather than always making new objects
    # [+name+] Name of the analysis represented by this AnalysisEntity.
    # [+type+] Type of the analysis represented by this AnalysisEntity.
    # [+dataLevel+]
    # [+experiment+] The experiment for which this analysis was run.
    # [+state+] Integer based bit-flag for multiple states.

    def update(name, type, dataLevel, experiment, state, avpHash)
      # Clear old values
      @refs.clear() if(@refs)
      @avpHash = if(avpHash.nil?) then Hash.new else avpHash end
      # Set class variables to new values
      @name, @type, @dataLevel, @experiment, @state = name, type, dataLevel, experiment, state
    end
    ...
    # [+returns+] Tab delimited analysis information
    def to_tabbed(attrNames=nil)
      ...
      retVal << "#{esc(@name)}\t#{esc(@type)}\t#{esc(@dataLevel)}\t#{esc(@experiment)}\t#{esc(@state)}"
    ...
    def self.from_tabbed(data, opts={})
      retVal = AnalysisEntityList.from_tabbed(data, opts)
  ...
  end # class AnalysisEntity < BRL::Genboree::REST::Data::AbstractEntity

  # AnalysisEntityList - Collection/list containing multiple AnalysisEntities.
  # NOTE: the elements of the list are AnalysisEntity objects. Inputs are
  #   assumed to correctly be instances of AnalysisEntity.
  #
  # Common Interface Methods:
  # - #to_json, #to_yaml, #to_xml        -- default implementations from parent class
  # - #getFormatableDataStruct           -- default implementations from parent class
  class AnalysisEntityList < BRL::Genboree::REST::Data::EntityList
    # What formats are supported, using the conventional +Symbols+. Subclasses may or may not override this.
    FORMATS = [ :JSON, :JSON_PRETTY, :XML, :YAML, :TABBED ]
    ...
    def to_tabbed()
      ...
      retVal << "##{AnalysisEntity::TABBED_HEADER.join("\t")}"
    ...
    def self.from_tabbed(data, opts={})
      header = nil
      retVal = AnalysisEntityList.new(false)
      data.each_line{ |line|
        ...
        if(header.nil?)
          ...
          retVal << AnalysisEntity.new(false, *values)
        end
      }
  ...
  end # class AnalysisEntityList < BRL::Genboree::REST::Data::EntityList
