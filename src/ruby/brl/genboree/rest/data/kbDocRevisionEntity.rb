...
module BRL ; module Genboree ; module REST ; module Data
  ...
  class KbDocRevisionEntity < BRL::Genboree::REST::Data::KbDocEntity
    ...
    def getFormatableDataStruct()
      ...
      kbDoc.delProp("revisionNum.properties.docRef") rescue nil
      kbDoc.delProp("revisionNum.properties.content.value._id") rescue nil
      # Delete the subDocPath since every record will pertain to the same subDocPath (even if it's been extracted from a parent property which can lead to confusion)
      kbDoc.delProp("revisionNum.properties.subDocPath") rescue nil
    ...
    # Inherited revision works by using SIMPLE_FIELD_NAMES and reflection methods; even if you
    ...
    def toStructuredData(wrap=@doWrap)
      ...
      # in addition to parent's object cleaning, delete nested _id and other doc refs
      kbDoc = BRL::Genboree::KB::KbDoc.new(data)
      docRef = kbDoc.delProp("revisionNum.docRef") rescue nil
      # Delete the subDocPath since every record will pertain to the same subDocPath (even if it's been extracted from a parent property which can lead to confusion)
      docRef = kbDoc.delProp("revisionNum.subDocPath") rescue nil
      contentProp = kbDoc.getPropVal("revisionNum.content")
  ...
  end # class KbDocRevisionEntity < BRL::Genboree::REST::Data::AbstractEntity

  class KbDocRevisionEntityList < BRL::Genboree::REST::Data::KbDocEntityList
    # A +Symbol+ naming the resource with a code-friendly (ok, XML tag-name friendly) name. This is currently used in
    # generating XML for _lists_...they will be lists of tags of this type (XML makes it hard to express certain natural things easily...)
    RESOURCE_TYPE = :KbDocRevisionList
    ...
    ELEMENT_CLASS = KbDocRevisionEntity
  end # class KbDocRevisionEntityList < BRL::Genboree::REST::Data::EntityList
