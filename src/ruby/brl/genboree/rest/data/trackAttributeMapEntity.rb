...
module BRL ; module Genboree ; module REST ; module Data
  # Hash of track names to simple attribute-value pairs

  # TrackAttributeMapEntity - Object containing a simple Hash of track names to simple attribute-value pairs
  #
  ...
  class TrackAttributeMapEntity < BRL::Genboree::REST::Data::EntityHash
    ...
    def self.from_tabbed(data, opts={})
      ...
      data.each_line { |line|
        ...
        if(header.nil?)
          ...
          dataLine = line.split(/\t/)
          raise BRL::Genboree::GenboreeError.new(:'Unsupported Media Type', "Wrong number of columns on line #{lineno}. Header indicates #{header.size} columns. Sample record at line #{lineno} has #{dataLine.size} columns. Aborting import of track attributes.") unless(headerSize == dataLine.size)
          trkName = dataLine[nameIndex]
          tmpHash = mapHash[trkName]
      ...
      retVal = TrackAttributeMapEntity.new(false, mapHash)
      # Now return our hash
      return retVal
    end
  end # class TrackAttributeMapEntity < BRL::Genboree::REST::Data::EntityList
