...
module BRL ; module Genboree ; module REST ; module Data
  ...
  # StudyEntity - Representation of a study
  #
  ...
  class StudyEntity < BRL::Genboree::REST::Data::AbstractEntity
    ...
    SIMPLE_FIELD_NAMES = [ "name", "type", "lab", "contributors", "state", "avpHash" ]
    ...
    # columns.
    TABBED_HEADER = [ "name", "type", "lab", "contributors", "state"]
    # Name of this Study
    attr_accessor :name
    # +Type+
    attr_accessor :type
    ...
    # +State+
    attr_accessor :state
    # +avpHash+ All of the attribute value pairs for this Study, in a Hash
    ...
    def initialize(doRefs=true, name="", type="", lab="", contributors="", state="", avpHash=Hash.new)
      super(doRefs)
      self.update(name, type, lab, contributors, state, avpHash)
    end
    ...
    def update(name, type, lab, contributors, state, avpHash)
      # Clear old values
      @refs.clear() if(@refs)
      if(avpHash.nil?)
        @avpHash = Hash.new
      ...
      @name, @type, @lab, @contributors, @state = name, type, lab, contributors, state
    end
    ...
    # [+returns+] Tab delimited study information
    def to_tabbed(attrNames=nil)
      ...
      retVal << "#{esc(@name)}\t#{esc(@type)}\t#{esc(@lab)}\t#{esc(@contributors)}\t#{esc(@state)}"
    ...
    def self.from_tabbed(data, opts={})
      retVal = StudyEntityList.from_tabbed(data, opts)
  ...
  end # class StudyEntity < BRL::Genboree::REST::Data::AbstractEntity

  # StudyEntityList - Collection/list containing multiple saved study
  # objects.
  # NOTE: the elements of the list are StudyEntity objects. Inputs are
  #   assumed to correctly be instances of StudyEntity.
  #
  # Common Interface Methods:
  # - #to_json, #to_yaml, #to_xml        -- default implementations from parent class
  # - AbstractEntity.from_json, AbstractEntity.from_yaml, AbstractEntity.from_xml  -- default implementations from parent class
  # - #getFormatableDataStruct           -- default implementations from parent class
  class StudyEntityList < BRL::Genboree::REST::Data::EntityList
    # What formats are supported, using the conventional +Symbols+. Subclasses may or may not override this.
    FORMATS = [ :JSON, :JSON_PRETTY, :XML, :YAML, :TABBED ]
    # A +Symbol+ naming the resource with a code-friendly (ok, XML tag-name friendly) name. This is currently used in
    # generating XML for _lists_...they will be lists of tags of this type (XML makes it hard to express certain natural things easily...)
    RESOURCE_TYPE = :StudyList
    ...
    def to_tabbed()
      ...
      retVal << "##{StudyEntity::TABBED_HEADER.join("\t")}"
    ...
    def self.from_tabbed(data, opts={})
      header = nil
      retVal = StudyEntityList.new(false)
      data.each_line{ |line|
        ...
        if(header.nil?)
          ...
          retVal << StudyEntity.new(false, *values)
        end
      }
  ...
  end # class StudyEntityList < BRL::Genboree::REST::Data::EntityList
