...
module BRL ; module Genboree ; module REST ; module Data
  ...
  class RunEntity < BRL::Genboree::REST::Data::AbstractEntity
    ...
    SIMPLE_FIELD_NAMES = [ "name", "type", "time", "performer", "location" , "experiment", "state", "avpHash" ]
    ...
    # columns.
    TABBED_HEADER = [ "name", "type", "time", "performer", "location" , "experiment", "state" ]
    # Name of this Run
    attr_accessor :name
    # +Type+
    attr_accessor :type
    ...
    # +State+
    attr_accessor :state
    # +avpHash+ All of the attribute value pairs for this Run, in a Hash
    ...
    # [+time+] [optional; default=empty]
    # [+experiment+] [optional; default=""]
    # [+state+] [optional; default=""]
    def initialize(doRefs=true, name="", type="", time="", performer="", location="", experiment="", state="", avpHash=Hash.new)
      super(doRefs)
      self.update(name, type, time, performer, location, experiment, state, avpHash)
    end
    ...
    def update(name, type, time, performer, location, experiment, state, avpHash)
      # Clear old values
      @refs.clear() if(@refs)
      @avpHash = if(avpHash.nil?) then Hash.new else avpHash end
      # Set class variables to new values
      @name, @type, @performer, @location, @experiment, @state = name, type, performer, location,  experiment, state
      self.time=(time)
    end
    ...
    def getFormatableDataStruct()
      data =  {
                "name" => @name,
                "type" => @type,
                "time" => (@time.respond_to?(:rfc822) ? @time.rfc822 : @time.to_s),
                "performer" => @performer,
                "location" => @location,
    ...
    def toStructuredData(wrap=@doWrap)
      ...
      data =
      {
        "name" => @name,
        "type" => @type,
        "time" => (@time.respond_to?(:rfc822) ? @time.rfc822 : @time.to_s),         # Should already be a String though
        "performer" => @performer,
        "location" => @location,
        "experiment" => @experiment,
        "state" => @state,
    ...
    # [+returns+] Tab delimited run information
    def to_tabbed(attrNames=nil)
      ...
      retVal << "#{esc(@name)}\t#{esc(@type)}\t#{esc(@time)}\t#{esc(@perfomer)}\t#{esc(@location)}\t#{esc(@experiment)}\t#{esc(@state)}"
    ...
    def self.from_tabbed(data, opts={})
      retVal = RunEntityList.from_tabbed(data, opts)
  ...
  end # class RunEntity < BRL::Genboree::REST::Data::AbstractEntity

  # RunEntityList - Collection/list containing multiple run objects.
  # NOTE: the elements of the list are RunEntity objects. Inputs are
  #   assumed to correctly be instances of RunEntity.
  #
  # Common Interface Methods:
  # - #to_json, #to_yaml, #to_xml        -- default implementations from parent class
  # - #getFormatableDataStruct           -- default implementations from parent class
  class RunEntityList < BRL::Genboree::REST::Data::EntityList
    # What formats are supported, using the conventional +Symbols+. Subclasses may or may not override this.
    FORMATS = [ :JSON, :JSON_PRETTY, :XML, :YAML, :TABBED ]
    # A +Symbol+ naming the resource with a code-friendly (ok, XML tag-name friendly) name. This is currently used in
    # generating XML for _lists_...they will be lists of tags of this type (XML makes it hard to express certain natural things easily...)
    RESOURCE_TYPE = :RunList
    ...
    def to_tabbed()
      ...
      retVal << "##{RunEntity::TABBED_HEADER.join("\t")}"
    ...
    def self.from_tabbed(data, opts={})
      header = nil
      retVal = RunEntityList.new(false)
      data.each_line{ |line|
        ...
        if(header.nil?)
          ...
          header.each{ |key|
            ...
            elsif(key == "experiment")
              values[5] = dataLine[idx]
            elsif(key == "state")
              values[6] = dataLine[idx]
            else
              raise "Duplicate columns in the header" unless(values[7][key].nil?)
              values[7][key] = dataLine[idx] unless(dataLine[idx].empty?)
            end
          }
          retVal << RunEntity.new(false, *values)
        end
      }
  ...
  end # class RunEntityList < BRL::Genboree::REST::Data::EntityList
