...
#++
  ...
  class TrackIsTemplate < BRL::REST::Resources::GenboreeResource
    ...
    TEMPLATE_URI = "/REST/v1/grp/{grp}/db/{db}/trk/{trk}/isTemplate"
    ...
    def self.pattern()
      ...
      return %r{^/REST/#{VER_STR}/grp/([^/\?]+)/db/([^/\?]+)/(?:(?:trk/([^/\?]+))|(?:trks))/isTemplate$}
    end
    ...
    def prepResponse()
      ...
      #$stderr.puts "val: #{val["#{@trackName}"].inspect}"
      isTempl = false
      dbNames = val["#{@trackName}"]["dbNames"]
      if(dbNames.size > 1)
        isTempl = true
      elsif(dbNames[0]['dbType'] == :sharedDb)
        isTempl = true
      end
      #$stderr.puts("dbNames[0]: #{dbNames[0]['dbType'].inspect}")
      entity = BRL::Genboree::REST::Data::TextEntity.new(@connect, isTempl)
      entity.makeRefsHash("/REST/#{VER_STR}/grp/#{Rack::Utils.escape(@groupName)}/db/#{Rack::Utils.escape(@dbName)}/trk/#{Rack::Utils.escape(@trackName)}/isTemplate")
