...
#++
  ...
  # StudyAttributes - exposes information about the specific attributes
  # of a study including the AVP pair information.
  #
  # Data representation classes used:
  # * BRL::Genboree::REST::Data::TextEntity
  # * BRL::Genboree::REST::Data::StudyEntity
  class StudyAttributes < BRL::REST::Resources::GenboreeResource
    ...
    RSRC_STRS = { :type => 'study', :label => 'study', :capital => 'Study', :pluralType => 'studies', :pluralLabel => 'studies', :pluralCap => 'Studies' }
    # Standard (mainly table-column) attrs
    STD_ATTRS = { 'name' => true, 'state' => true, 'lab' => true, 'contributors' => true, 'type' => true }
    ...
    def self.pattern()
      # Look for /REST/v1/grp/{grp}/db/{db}/study/{study}/attribute/{attr} URIs
    ...
    def updateEntityStdAttrs(entityId, attrValMap)
      return @dbu.updateStudyById(entityId, attrValMap['name'], attrValMap['type'], attrValMap['lab'], attrValMap['contributors'], attrValMap['state'])
    end

    # ATTR INTERFACE: Calls appropriate row-select-by @entityName method of dbu
    def deleteEntity2AttributeById(entityId, attrId)
      return @dbu.deleteStudy2AttributesByStudyIdAndAttrNameId(entityId, attrId)
    end
