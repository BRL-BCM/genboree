...
#++
  ...
  # TrackImporterInfoClasses - exposes the unique list of lffClasses found in the track importer info file.
  #
  # Data representation classes used:
  # * BRL::Genboree::REST::Data::TextEntity
  class TrackImporterInfoClasses < BRL::REST::Resources::GenboreeResource
    ...
    # [+returns+] This +Regexp+: <tt>^/REST/#{VER_STR}/resources/importer/build/([^/\?]+)/src/([^/\?]+)/classes</tt>
    def self.pattern()
      return %r{^/REST/#{VER_STR}/resources/importer/build/([^/\?]+)/src/([^/\?]+)/classes}      # Look for /REST/v1/resources/importer/build/([^/\?]+)/src/{src}/classes
    end
    ...
    def setResponse(statusName=:OK, statusMsg='')
      begin
        ...
        # Get classes
        classes = importInfo.classes(@src)
        # Transform classes into return data
        bodyData = BRL::Genboree::REST::Data::TextEntityList.new(false)
        classes.each { |lffClass|
          entity = BRL::Genboree::REST::Data::TextEntity.new(false, lffClass)
          bodyData << entity
        }
  ...
  end # class TrackImporterInfoClasses
