...
#++
  ...
  # ProjectAdditionalFiles - for putting/getting/deleting files to the 'genb^^additionalFiles' dir of a project.
  #
  class ProjectAdditionalFiles < BRL::REST::Resources::GenboreeResource
    ...
    RSRC_TYPE = 'projectAdditionalFiles'
    ...
    # [+returns+] This +Regexp+: <tt>^/REST/#{VER_STR}/grp/([^/\?]+)/prj/([^/\?]+)/additionalFiles/file/([^\?]+)(?:\?.*)?$</tt>
    def self.pattern()
      return %r{^/REST/#{VER_STR}/grp/([^/\?]+)/prj/([^/\?]+)/additionalFiles/file/([^\?]+)(?:\?.*)?$}       # Look for /REST/v1/group/{grp}/prj/{prj}/additionalFiles/file/{path to file} URIs
    end
    ...
    def initOperation()
      ...
      if(initStatus == :OK)
        ...
        if(!File.exists?(@prjFileBase))
          ...
          @additionalFilesDir = "#{@prjFileBase}/genb^^additionalFiles"
          FileUtils.mkdir_p(@additionalFilesDir)
          @filesDir = Dir.new(@additionalFilesDir)
        end
      end
      return initStatus
    end
    ...
    def getFullFilePath(fileName=@fileName)
      ...
      if(safeFileName and !safeFileName.empty?)
        retVal = "#{@additionalFilesDir}/#{safeFileName}"
      end
      return retVal
    end
      ...
      if(initStatus == :OK)
        begin
          ...
          if(!fullFilePath.empty? and File.exist?(fullFilePath))
            ...
            @statusMsg = "NOT FOUND: The file #{@fileName.inspect} (under genb^^additionalFiles) could not be found for project #{@prjName.inspect} in user group #{@groupName.inspect}."
          end
    ...
    def put()
      ...
      if(initStatus == :OK)
        begin
          ...
          respEntity = BRL::Genboree::REST::Data::AbstractEntity.new(false, true, :Accepted, msg="ACCEPTED: Upload of file to Project (under genb^^additionalFiles) #{@prjName.inspect}, file: #{@fileName.inspect}, accepted. Final storage of file may be ongoing.")
