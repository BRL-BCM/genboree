...
module BRL ; module Genboree ; module REST ; module Helpers
  class TrackEntityListApiUriHelper < ApiUriHelper
    # Each resource specific API Uri Helper subclass should redefine this:
    NAME_EXTRACTOR_REGEXP = %r{^http://[^/]+/REST/v\d+/grp/[^/]+/db/[^/]+/trks/entityList/([^/\?]+)}
    EXTRACT_SELF_URI = %r{^(.+?/trks/entityList/[^/\?]+)}     # To get just this resource's portion of the URL, with any suffix stripped off
