...
module BRL ; module Genboree ; module REST ; module Helpers
  class ClassApiUriHelper < ApiUriHelper
    # Each resource specific API Uri Helper subclass should redefine this:
    NAME_EXTRACTOR_REGEXP = %r{^http://[^/]+/REST/v\d+/grp/[^/]+/db/[^/]+/class/([^/\?]+)}
    EXTRACT_SELF_URI = %r{^(.+?/class/[^/\?]+)}     # To get just this resource's portion of the URL, with any suffix stripped off
