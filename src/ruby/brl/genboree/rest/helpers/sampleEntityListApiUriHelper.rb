...
module BRL ; module Genboree ; module REST ; module Helpers
  class SampleEntityListApiUriHelper < ApiUriHelper
    # Each resource specific API Uri Helper subclass should redefine this:
    NAME_EXTRACTOR_REGEXP = %r{^http://[^/]+/REST/v\d+/grp/[^/]+/db/[^/]+/samples/entityList/([^/\?]+)}
    EXTRACT_SELF_URI = %r{^(.+?/samples/entityList/[^/\?]+)}     # To get just this resource's portion of the URL, with any suffix stripped off
