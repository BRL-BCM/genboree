...
class RunWigToBigWig
  ...
  def runApp()
    ...
    begin
      ...
      $stderr.puts "Downloading wig file for track #{@trackName} from host: #{@rsrcHost}"
      rsrcPath = "/REST/v1/grp/#{CGI.escape(@groupName)}/db/#{CGI.escape(@dbName)}/trk/#{CGI.escape(@trackName)}/annos?format=bedGraph&modulusLastSpan=true&spanAggFunction=avg" # 'modulusLastSpan=true' ensures we don't run off the end of the chromosome
      apiCaller = ApiCaller.new(@rsrcHost, rsrcPath, @user, @pwd)
      ...
      apiCaller = ApiCaller.new(@rsrcHost, "/REST/v1/grp/#{CGI.escape(@groupName)}/db/#{CGI.escape(@dbName)}/eps?", @user, @pwd)
      apiCaller.get()
      ...
      if(exitStatus and File.exists?(@bigFile))
        header = "success"
        # Copy over the bigwig file to the server by making an API call
        apiCaller = ApiCaller.new(@rsrcHost, "/REST/v1/grp/#{CGI.escape(@groupName)}/db/#{CGI.escape(@dbName)}/file/#{CGI.escape(@bigFile)}/data?fileType=bigFile&trackName=#{CGI.escape(@trackName)}", @user, @pwd)
        apiCaller.put({}, File.open(@bigFile))
        if(!apiCaller.succeeded?)
          header = 'failed'
          messBody = "Failed to put bigwig file using API.\nError Message:\n#{apiCaller.respBody.inspect}"
          raise "Api call to put .bw file failed.\nAPI Response: #{apiCaller.respBody.inspect}"
        else
          $stderr.debugPuts(__FILE__, __method__, "STATUS", "bigwig file successfully transferred to server using API. Removing bigwig file from local scratch dir.")
          `rm -f #{@bigFile}`
        end
      else
        header = "failed"
  ...
  def makeMessageBody(exitStatus, bigFile)
    ...
    if(exitStatus and File.exists?(bigFile))
      ...
      body << "http://#{@rsrcHost}/REST/#{BRL::REST::Resource::VER_STR}/grp/#{CGI.escape(@groupName)}/db/#{CGI.escape(@dbName)}/trk/#{CGI.escape(@trackName)}?gbKey=#{CGI.escape(@gbKey)}&format=ucsc_browser&ucscType=bigWig&ucscSafe=on\n\n"
      if(@genomeTemplate =~ /^\S/)
