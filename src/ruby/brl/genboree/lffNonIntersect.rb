...
class LffNonIntersect
  ...
  def setParameters()
		...
		@secondTrack =" -D #{CGI.escape(@optsHash['--secondOperandTrack'])} "
  ...
  def work()
		lffNonIntersectExeCommand = "lffIntersect.exe "
		lffNonIntersectExeCommand << @firstTrack << @secondTrack << @listOfLffFiles << @outputFile
		lffNonIntersectExeCommand << @newTrack << @newClass << @radius << @minTracks << @allTracks

		$stderr.puts "#{Time.now()} RUBY - TRACK OP: Executing command #{lffNonIntersectExeCommand}"
		exeOut = `#{lffNonIntersectExeCommand} 2>&1 `
		@exeStatus = $? ? $?.exitstatus : 123
  ...
  def LffNonIntersect.processArguments()
		...
		LffNonIntersect.usage() if(optsHash.key?('--help'));

		unless(progOpts.getMissingOptions().empty?)
			LffNonIntersect.usage("USAGE ERROR: some required arguments are missing")
		end

		LffNonIntersect.usage() if(optsHash.empty?);
		return optsHash
	end

	def LffNonIntersect.usage(msg='')
			unless(msg.empty?)
				puts "\n#{msg}\n"
			end
			puts "
  PROGRAM DESCRIPTION:
    ...
    containing only those annotations from the first operand that do not overlap
    with annotations from the second operand track.
    ...
    cases (eg, my ESTs that don't intersect with ESTs from any one of various
    cancer libraries). This is OPTIONAL.

		By default, first operand track annotations are not output if they intersect
		*any* of the other operand tracks. But if you use -a, you can require
		...
		track is rejected. This is OPTIONAL.

    Furthermore, you can also tell it a minimum number of intersections need be
    found before the record is eliminated. (eg, remove my ESTs that intersect
    with at least 3 ESTs from any one of various cancer libraries). If you have
    ...
    COMMAND LINE ARGUMENTS:
      -f    => Name of the first operand track. You will be precipitating out
               annotations from this track that do not overlap with annotations from
               the other track(s).
      -s    => Name(s) of the second operand track(s). If you specify more than
               one, then an annotation in the first track overlay an
               annotation in *any* of these tracks will NOT be output.
      ...
      -n    => New track name for annotations not having intersection.
    ...
    USAGE:
    lffNonIntersect.rb -f ESTs%3AUt1 -s Gene%3ARefSeq%2CGene%3AEns -l \
...
optsHash = LffNonIntersect.processArguments()
# Instantiate analyzer using the program arguments
lffNonIntersecter = LffNonIntersect.new(optsHash)
# Analyze this !
lffNonIntersecter.work()
exit(lffNonIntersecter.exeStatus) ;
