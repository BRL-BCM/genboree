...
#
# Notes:
#  - Can be run only in an env with the unix sort command
#  - Assumes the file is validated.
#  - Assumes that the file is uncompressed
#  - Unsorting variable step file(s) involves two steps since there are two levels of unsorting required for
#   variable step files
...
class Sorter
  ...
  # A hash to indicate all the file formats supported by Genboree
  FILETYPEHASH = {
    FIXEDSTEP => nil,
    VARIABLESTEP => nil,
  ...
  # Calling this method first is recommended before calling sortVariableStepBlocks()
  # [+returns+] nil
      ...
      buffIO.each_line { |line| 
        ...
        if(line =~ /\n$/)
          ...
          if(line =~ /^variableStep/)
            ...
            if(blockCount > 0)
              system("echo #{blockBuffer} >> #{fileWithSortedRecordsForEachBlock}") if(!blockBuffer.empty?)
              # Sort using unix 'sort'
              system("cut -f 1,2 #{fileWithSortedRecordsForEachBlock} | sort -d >> #{@sortedFile}")
              system("echo '' > #{fileWithSortedRecordsForEachBlock}")
              blockBuffer = ""
            end
            # Write block header to what is going to be the sorted version of the file
            system("echo #{line} >> #{@sortedFile}") 
            blockCount += 1
          elsif(line =~ /^(?:(\+|\-)?\d+)\s+(?:(?:\+|\-)?\d*(?:\d+\.|\.\d+)?(?:e(?:\+|\-)?\d+)?)$/i) # This regular expression checks for any number (including exponential values)
            blockBuffer << line
          end
          # print to file if blockBuffer is large enough
          if(blockBuffer.size >= MAXIMUMOUTPUTBUFFER)
            system("echo #{blockBuffer} >> #{fileWithSortedRecordsForEachBlock}")
            blockBuffer = ""
          end
        # save line as orphan  
        else
  ...
  def sortVariableStepBlocks()
          ...
          if(line =~ /^variableStep/)
            ...
            if(startCoord.nil?)
              ...
              if(previousOffset.nil?)
                ...
                writeBuffer << "#{tempOffset - previousOffset}\n#{startCoord}\t#{blockHeader}"
              end
    ...
    writeBuffer << "#{fileOffset} - #{previousOffset}\n"
    unsortedTabDelimitedFileWriter.print(writeBuffer)
    unsortedTabDelimitedFileWriter.close()
    sortedTabDelimitedFile = "#{@dirToUse}/#{Time.now().to_f}_#{SORTEDTABDELIMITEDFILE}"
    # Now call the unix sort command and sort the unsorted tab delimited file by the first column (coordinate)
  ...
  # Writes the entite wiggle (sorted) using the sorted tab delimited file reader
        ...
        if(tabDelimitedFileLine =~ /\n$/)
          ...
          if(blockSizeInBytes <= BUFFERSIZE)
            ...
            while(!(bytesProcessed >= blockSizeInBytes))
              fileBuffer = reader.read(bytesToProcess)
