...
module BRL ; module Genboree ; module Tools
  class EpigenomicFeaturesEnrichmentToolJobHelper < WorkbenchJobHelper

    TOOL_ID = 'epigenomicFeaturesEnrichmentTool'
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "#{@genbConf.toolScriptPrefix}wrapperPeaksEnrichment.rb"
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      workbenchJobObj.settings['enrichmentGraph'] = workbenchJobObj.settings['enrichmentGraph'] ? true : false
      uri = URI.parse(output)
      @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/#{CGI.escape("Structural Variation")}/EpigenomicEnrichment/#{CGI.escape(analysisName)}/jobFile.json/data?"
