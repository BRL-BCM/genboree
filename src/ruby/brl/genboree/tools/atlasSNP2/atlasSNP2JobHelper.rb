...
module BRL ; module Genboree ; module Tools
  class AtlasSNP2JobHelper < WorkbenchJobHelper
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "atlasSNP2Wrapper.rb"
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      workbenchJobObj.settings['removeDup'] = workbenchJobObj.settings['removeDup'] ? true : false
      workbenchJobObj.settings['keepOnlyUniqueMappings'] = workbenchJobObj.settings['keepOnlyUniqueMappings'] ? true : false
      workbenchJobObj.settings['separateSNPs'] = workbenchJobObj.settings['separateSNPs'] ? true : false
      uri = URI.parse(output)
      @jobFileCopyUriPaths = "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/#{CGI.escape("Atlas2 Suite Results")}/#{CGI.escape(studyName)}/Atlas-SNP2/#{CGI.escape(jobName)}/jobFile.json/data?"
