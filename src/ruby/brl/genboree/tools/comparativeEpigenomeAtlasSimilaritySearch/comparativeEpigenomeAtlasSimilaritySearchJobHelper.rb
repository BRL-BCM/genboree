...
module BRL ; module Genboree ; module Tools
  class ComparativeEpigenomeAtlasSimilaritySearchJobHelper < WorkbenchJobHelper
    TOOL_LABEL = :hidden
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      outputs.each { |output|
        if(@dbApiHelper.dbVersion(output) == @dbApiHelper.dbVersion(inputs[0])) # inputs[0] is always 'dependent'
          ...
          epiAtlasScrTracks = workbenchJobObj.settings['epiAtlasScrTracks']
          baseTrkUri = nil
          ...
          epiAtlasScrTracks.each { |trk|
            inputs.push("#{baseTrkUri}/trk/#{trk}?")
          }
        end
      }
