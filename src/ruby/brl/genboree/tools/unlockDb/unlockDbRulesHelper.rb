...
module BRL ; module Genboree ; module Tools
  class UnlockDbRulesHelper < WorkbenchRulesHelper
    ...
    def rulesSatisfied?(wbJobEntity, sectionsToSatisfy=[ :inputs, :outputs, :context, :settings ], toolIdStr=@toolIdStr)
      ...
      if(rulesSatisfied)
        ...
        if(!canonicalAddressesMatch?(URI.parse(wbJobEntity.outputs[0]).host, [@genbConf.machineName, @genbConf.machineNameAlias]))
          ...
          unless(permission)
            rulesSatisfied = false
            wbJobEntity.context['wbErrorMsg'] = "NO PERMISSION: You need administrator level access to unlock databases."
          else
            # ------------------------------------------------------------------
            # CHECK SETTINGS
            # ------------------------------------------------------------------
            ...
            unless(wbJobEntity.settings.key?('gbKey'))
              userId = wbJobEntity.context['userId']
              # Check if db already is unlocked
              uriObj = URI.parse(wbJobEntity.outputs[0])
              apiCaller = WrapperApiCaller.new(uriObj.host, "#{uriObj.path}", userId)
              apiCaller.initInternalRequest(@rackEnv, @genbConf.machineNameAlias) if(@rackEnv)
              apiCaller.get()
              resp = JSON.parse(apiCaller.respBody)['data']
              wbJobEntity.settings['gbKey'] = resp['gbKey']
            end
          end
        end
      end
