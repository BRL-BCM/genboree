...
module BRL ; module Genboree ; module Tools
  class RdpRulesHelper < WorkbenchRulesHelper
    ...
    def rulesSatisfied?(wbJobEntity, sectionsToSatisfy=[ :inputs, :outputs, :context, :settings ], toolIdStr=@toolIdStr)
      ...
      if(rulesSatisfied)
        ...
        if(sectionsToSatisfy.include?(:settings))
          ...
          # Check 1: does the dob dir already exist?
          ...
          rcscUri << "/file/MicrobiomeWorkBench/#{CGI.escape(wbJobEntity.settings['studyName'])}/RDP/#{CGI.escape(wbJobEntity.settings['jobName'])}/jobFile.json?"
