...
module BRL ; module Genboree ; module Tools
  class FileTextViewerJobHelper < WorkbenchJobHelper
    ...
    def runInProcess()
      ...
      inputs.each { |file|
        ...
        rsrcPath << "/data?format=preHtml&maxBytes=#{@workbenchJobObj.settings['maxBytes']}"
        ...
        if (!apiCaller.succeeded?)
          ...
          success = false
        # Otherwise, if the API call did succeed, then we will set the 'textResponse' field of our settings to be the requested text
        else
          @workbenchJobObj.settings['textResponse'] = apiCaller.respBody
        end
      }
