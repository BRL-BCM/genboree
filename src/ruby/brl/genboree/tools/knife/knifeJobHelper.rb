...
module BRL ; module Genboree ; module Tools
  class KnifeJobHelper < WorkbenchJobHelper
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "knifeWrapper.rb"
    end
   ...
   # Casts certain args to the tool to integer
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      if(remoteStorageArea)
        @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/#{CGI.escape(remoteStorageArea)}/KNIFE_v#{@toolVersion}/#{CGI.escape(analysisName)}/jobFile.json/data?"
      else
        @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/KNIFE_v#{@toolVersion}/#{CGI.escape(analysisName)}/jobFile.json/data?"
      end
