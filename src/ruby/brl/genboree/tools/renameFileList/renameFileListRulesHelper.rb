...
module BRL ; module Genboree ; module Tools
  class RenameFileListRulesHelper < WorkbenchRulesHelper
    ...
    def rulesSatisfied?(wbJobEntity, sectionsToSatisfy=[ :inputs, :outputs, :context, :settings ], toolIdStr=@toolIdStr)
      ...
      if(sectionsToSatisfy.include?(:settings))
        ...
        fileEntityList = @fileEntityListApiHelper.extractName(output)
        apiCaller = ApiCaller.new(targetHost, "#{targetRsrcPath}/files/entityList/#{CGI.escape(newName)}?", @hostAuthMap)
        apiCaller.initInternalRequest(@rackEnv, @genbConf.machineNameAlias) if(@rackEnv)
        apiCaller.get()
        resp = JSON.parse(apiCaller.respBody)['data']
        if(!resp.empty?)
          wbJobEntity.context['wbErrorMsg'] = "ALREADY_EXISTS: A file entity list with the name: '#{newName}' already exists in the target database. "
        else
          rulesSatisfied = true
        end
      end
