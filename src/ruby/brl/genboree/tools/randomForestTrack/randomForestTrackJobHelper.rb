...
module BRL ; module Genboree ; module Tools
  class RandomForestTrackJobHelper < WorkbenchJobHelper

    TOOL_ID = 'randomForestTrack'


    # Instance vars that should be overridden to launch the job
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      super(toolIdStr, genbConf, dbu, args)
      self.class.commandName = "randomForestWrapper.rb"
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      if(settings.key?('userRoiTrk') and !settings['userRoiTrk'].empty?)
        settings['trkHash'].each_key { |input|
          if(input == settings['userRoiTrk'])
            roiFound = true
      ...
      if(!roiFound and workbenchJobObj.settings['repoRoiTrk'] and !workbenchJobObj.settings['repoRoiTrk'].empty?)
        workbenchJobObj.settings['roiTrack'] = "#{@genbConf.roiRepositoryGrp}#{workbenchJobObj.settings['dbVer']}/trk/#{CGI.escape(workbenchJobObj.settings['repoRoiTrk'])}?"
      else
      ...
      workbenchJobObj.settings.delete('attributesList') if(workbenchJobObj.settings.key?('attributesList'))
      workbenchJobObj.settings.delete('userRoi') if(workbenchJobObj.settings.key?('userRoi'))
      workbenchJobObj.settings.delete('trkHash') if(workbenchJobObj.settings.key?('trkHash'))
