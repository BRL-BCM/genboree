...
module BRL ; module Genboree ; module Tools
  class CuffdiffJobHelper < WorkbenchJobHelper
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "cuffdiffWrapper.rb"
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/Cuffdiff/#{CGI.escape(analysisName)}/jobFile.json/data?"
      ...
      workbenchJobObj.settings['timeSeries'] = workbenchJobObj.settings['timeSeries'] ? true : false
      workbenchJobObj.settings['poissonDisp'] = workbenchJobObj.settings['poissonDisp'] ? true : false
      if(workbenchJobObj.settings['entityListHash']) # For entity list inputs, replace 'sampleList' values with file names instead of file entities
        workbenchJobObj.settings['sampleList'] = workbenchJobObj.settings['entityListHash']
      end
