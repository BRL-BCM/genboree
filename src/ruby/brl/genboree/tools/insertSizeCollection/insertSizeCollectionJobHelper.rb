...
module BRL ; module Genboree ; module Tools
  class InsertSizeCollectionJobHelper < WorkbenchJobHelper
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "wrapperInsertSizeCollect.rb"
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      inputs.size.times { |ii|
        inputType.push(workbenchJobObj.settings["inputType_#{ii}"])
      }
      workbenchJobObj.settings['inputType'] = inputType
      uri = URI.parse(output)
      @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/#{CGI.escape("Structural Variation")}/InsertSizes/#{CGI.escape(analysisName)}/jobFile.json/data?"
