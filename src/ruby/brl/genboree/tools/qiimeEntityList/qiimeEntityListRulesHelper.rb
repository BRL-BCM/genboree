...
module BRL ; module Genboree ; module Tools
  class QiimeEntityListRulesHelper < WorkbenchRulesHelper
      ...
      if(rulesSatisfied)
        ...
        if(sectionsToSatisfy.include?(:settings))
          ...
          unless( sectionsToSatisfy.include?(:outputs) and  sectionsToSatisfy.include?(:inputs) )
            ...
            # Min Value Count should be an integer
            minValueCount = String.new(wbJobEntity.settings['minValueCount'].strip)
            if(!minValueCount.valid?(:int))
              rulesSatisfied = false
              wbJobEntity.context['wbErrorMsg'] = "INVALID_INPUT: Min Value Count needs to be an integer value."
            end
          end
        end
      end
