...
module BRL ; module Genboree ; module Tools
  class RemoveFilesJobHelper < WorkbenchJobHelper
    ...
    def runInProcess()
      ...
      inputs.each { |file|
        ...
        if(apiCaller.succeeded?)
          ...
          problemList.push(@fileApiHelper.extractName(file))
        end
        nodeCount += 1
      }
      if(!problemList.empty?)
        success = false
        @workbenchJobObj.context['wbErrorMsg'] = "The following files/folders could not be deleted: #{problemList.join(",")}."
      end
      return success
    end
    ...
    def getMessage(msgType, wbJobEntity)
      ...
      wbJobEntity.context['resourceType'] = 'files'
