...
module BRL ; module Genboree ; module Tools
  class TargetInteractionFinderJobHelper < WorkbenchJobHelper
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "targetInteractionFinderWrapper.rb"
    end
    ...
    def preCmds()
      return [
        "module load TargetInteractionFinder/1.0"
      ]
    end
   ...
   # Casts certain args to the tool to integer
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      unless(remoteStorageArea)
        @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/targetInteractionFinder_v#{@toolVersion}/#{CGI.escape(analysisName)}/jobFile.json/data?"
      else
        @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/#{remoteStorageArea}/targetInteractionFinder_v#{@toolVersion}/#{CGI.escape(analysisName)}/jobFile.json/data?"
      end
