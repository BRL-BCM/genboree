...
module BRL ; module Genboree ; module Tools
  class RemoveTracksFromEntityListRulesHelper < WorkbenchRulesHelper
        ...
        unless(checkDbVersions(inputs + outputs, skipNonDbUris=true)) # Failed
          ...
          if(sectionsToSatisfy.include?(:settings))
            if(wbJobEntity.settings['multiSelectInputList'].nil? or wbJobEntity.settings['multiSelectInputList'].empty?)
              wbJobEntity.context['wbErrorMsg'] = "INVALID_INPUT: You must select at least one track to remove from the entity list."
            else        
              rulesSatisfied = true
            end
          else
