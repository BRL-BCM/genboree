...
module BRL ; module Genboree ; module Tools
  class MrfAnnotationCoverageJobHelper < WorkbenchJobHelper

    TOOL_ID = 'mrfAnnotationCoverage'

    # Instance vars that should be overridden to launch the job
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      super(toolIdStr, genbConf, dbu, args)
      self.class.commandName = "mrfAnnotationCoverageWrapper.rb"
    end
...
#     workbenchJobObj.settings['gtfDir'] = "#{@genbConf.clusterGTFDir}"
      ...
      @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/MrfAnnotationCoverage/#{CGI.escape(analysisName)}/jobFile.json/data?"
