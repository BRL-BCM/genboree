...
module BRL ; module Genboree ; module Tools
  class ComparativeSignalDataComparisonRulesHelper < WorkbenchRulesHelper
    TOOL_LABEL = :hidden
    def rulesSatisfied?(wbJobEntity, sectionsToSatisfy=[ :inputs, :outputs, :context, :settings ], toolIdStr=@toolIdStr)
      ...
      if(rulesSatisfied)
        ...
        # ------------------------------------------------------------------
        # Check 0: If there are 3 inputs, one of them MUST be a folder
        folderPresent = false
        inputs.each { |input|
        ...
        if(!folderPresent and inputs.size == 3)
          wbJobEntity.context['wbErrorMsg'] = "INVALID_INPUT: In case of 3 inputs, one of them MUST be the output folder from the ROI-Lifter tools"
        else
          ...
          unless(@dbApiHelper.accessibleByUser?(outputs.first, userId, CAN_WRITE_CODES))
            ...
            unless(permission)
              ...
              # Check 3: if inputs.size == 3, i.e, two trks and one input folder, we need to make sure that the folder IS the output
              ...
              unless(jobFilePresent) # Failed.
                ...
                lifterOutputs = jobFileObj.outputs if(inputsSize == 3)
                if(inputsSize == 4)
                  if((@dbApiHelper.dbVersion(inputs[0]) != @dbApiHelper.dbVersion(inputs[1])) or (@dbApiHelper.dbVersion(inputs[2]) != @dbApiHelper.dbVersion(inputs[3]))) # Failed
                    correctOrder = false
                    errorMsg = 'INVALID_INPUT: The order of input tracks is incorrect. The database version of the first track MUST match the version of the second track and the database version
                              of the third track MUST match the version of the fourth track.'
                  end
                else # 2 tracks and one folder
                  if(
                      (@dbApiHelper.dbVersion(lifterOutputs[0]) != @dbApiHelper.dbVersion(inputs[depTrackOrder]) and @dbApiHelper.dbVersion(lifterOutputs[0]) != @dbApiHelper.dbVersion(inputs[indepTrackOrder])) or
                      (@dbApiHelper.dbVersion(lifterOutputs[1]) != @dbApiHelper.dbVersion(inputs[depTrackOrder]) and @dbApiHelper.dbVersion(lifterOutputs[1]) != @dbApiHelper.dbVersion(inputs[indepTrackOrder]))
                    )
                    correctOrder = false
                    errorMsg = "INVALID_INPUT: The database version of one of the ROI Tracks from the ROI-Lifter job MUST match one of the input score tracks and the other ROI Track MUST match the other score track."
                  end
                end
                unless(correctOrder)
                  ...
                  if(sameDbVer)
                    ...
                    if(inputsSize == 4)
                      scoreTrackHighDens = false if(!@trkApiHelper.isHdhv?(inputs[1]) or !@trkApiHelper.isHdhv?(inputs[3]))
                    else
                      scoreTrackHighDens = false if(!@trkApiHelper.isHdhv?(inputs[depTrackOrder]) or !@trkApiHelper.isHdhv?(inputs[indepTrackOrder]))
                    end
                    unless(scoreTrackHighDens)
                      wbJobEntity.context['wbErrorMsg'] = "INVALID_INPUT: Both of the score tracks MUST be high density score tracks. "
                    else
                      ...
                      if(roiTrackHighDens)
                        ...
                        if(outputsSize == 2 and (@dbApiHelper.dbVersion(outputs[0]) == @dbApiHelper.dbVersion(outputs[1])))
                          ...
                          unless(outputMatch)
                            ...
                            if(outputsSize == 1)
                              if(inputsSize == 4)
                                independentMatch = false if(@dbApiHelper.dbVersion(outputs[0]) != @dbApiHelper.dbVersion(inputs[1]))
                              else
                                independentMatch = false if(@dbApiHelper.dbVersion(outputs[0]) != @dbApiHelper.dbVersion(inputs[depTrackOrder]))
                              end
                            end
                            unless(independentMatch)
                              wbJobEntity.context['wbErrorMsg'] = "INVALID_INPUT: The version of the target database MUST match the version of the 'dependent' set of tracks. "
                            else
                              # If we have 3 inputs we need to overwrite wbJobEntity.inputs. We don't want to do this in the .rhtml for the sake of keeping the 'view' clean.
                              if(inputsSize == 3)
                                ...
                                if(@dbApiHelper.dbVersion(lifterOutputs[0]) == @dbApiHelper.dbVersion(inputs[depTrackOrder]))
                                  ...
                                  newInputs.push(inputs[indepTrackOrder])
                                else
        ...
        if(sectionsToSatisfy.include?(:settings))
          $stderr.puts "DEBUG: Checking settings..."
          ...
          if(jobFolderPresent)
            ...
            $stderr.puts "DEBUG: settings checked..."
            rulesSatisfied = true
          end
        end
      end
