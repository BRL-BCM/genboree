...
module BRL ; module Genboree ; module Tools
  class CopyTrackListsJobHelper < WorkbenchJobHelper
    ...
    def runInProcess()
      ...
      deleteSourceTracksRadio = CGI.escape(@workbenchJobObj.settings['deleteSourceTracksRadio'])
      ...
      multiSelectInputList.each { |input|
        ...
        trkEntityList = @trackEntityListApiHelper.extractName(input)
        # Do a 'put' on the target db:
        apiCaller = WrapperApiCaller.new(targetHost, "#{targetRsrcPath}/trks/entityList/#{CGI.escape(trkEntityList)}?", @userId)
        ...
        if(deleteSourceTracksRadio == 'move') # We need to nuke the source entity list if we are doing a move
          apiCallerSrc.delete()
        end
      }
      ...
      if(multiSelectInputList.size != listsCopied)
        success = false
        @workbenchJobObj.context['wbErrorMsg'] = "Only #{listsCopied} track entity list(s) were copied/moved."
      end
      return success
    end
