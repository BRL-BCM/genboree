...
module BRL; module Genboree; module Tools
  class ExogenousSTARMappingJobHelper < WorkbenchJobHelper
    TOOL_ID = "exogenousSTARMapping"

    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      super(toolIdStr, genbConf, dbu, args)
      self.class.commandName = "exogenousSTARMappingWrapper.rb"
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      unless(remoteStorageArea)
        @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/exceRptPipeline_v#{toolVersion}/#{CGI.escape(analysisName)}/jobFile_exogenous.json/data?"
      else
        @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/#{remoteStorageArea}/exceRptPipeline_v#{toolVersion}/#{CGI.escape(analysisName)}/jobFile_exogenous.json/data?"
      end
