...
module BRL ; module Genboree ; module Tools
  class CombinedCoverageJobHelper < WorkbenchJobHelper
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "combinedCoverageWrapper.rb"
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      # Clean up sampleName properly
      settings = workbenchJobObj.settings
      sampleType = []
      settings.keys.each { |key|
        next unless(key =~ /^sampleType_(\d+)$/)
        colIdx = $1.to_i
        sampleType[colIdx] = settings[key]
      }
      settings.delete_if { |key, val| (key =~ /^sampleType_/) }
      settings['sampleType'] = sampleType
