...
module BRL ; module Genboree ; module Tools
  class RseqtoolsRulesHelper < WorkbenchRulesHelper
    ...
    def rulesSatisfied?(wbJobEntity, sectionsToSatisfy=[ :inputs, :outputs, :context, :settings ], toolIdStr=@toolIdStr)
      ...
      if(rulesSatisfied)
        ...
        # ------------------------------------------------------------------
        # Check 1: Make sure the genome version is currently supported by RSEQtools
        # ------------------------------------------------------------------
        ...
        if(genomesSatisfied)  # If genome is not supported, then do not process any further
          ...
          if(expInputUri.scheme =~ /file/) # Input is a local file, so wrapper will handle this case
            rulesSatisfied = true
          else  # Input is a Genboree URL
          ...
          end # if(expInputUri.scheme =~ /file/)
        ...
        if(sectionsToSatisfy.include?(:settings))
          ...
          rcscUri << "/file/RSEQtools/#{CGI.escape(wbJobEntity.settings['analysisName'])}/jobFile.json?"
