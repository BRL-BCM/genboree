...
module BRL ; module Genboree ; module Tools
  class RseqtoolsJobHelper < WorkbenchJobHelper
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "rseqtoolsWrapper.rb"
    end
    ...
    def exports()
      return [
        ...
        "export RSEQTOOLS_ANNOS=#{@genbConf.clusterRseqtoolsAnnoDir}"
      ]
    end
    ...
    def preCmds()
      return [
        "module swap jdk/1.6",
        "module load bowtie/2.1",
        "module load RSeqTools/0.7"
      ]
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      if(apiUriHelper.extractType(workbenchJobObj.outputs[0]) != "db" )
        workbenchJobObj.outputs.reverse!
      end
      ...
      dbVer = settings['genomeVersion'] = @dbApiHelper.dbVersion(output, @hostAuthMap).downcase
      group = @grpApiHelper.extractName(output)
      db = @dbApiHelper.extractName(output)
      analysisName = settings['analysisName']
      uri = URI.parse(output)
      @jobFileCopyUriPaths << "http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/RSEQtools/#{CGI.escape(analysisName)}/jobFile.json/data?"
      settings['doUploadResults'] = settings['doUploadResults'] ? true : false

      ## Remove unwanted settings values
