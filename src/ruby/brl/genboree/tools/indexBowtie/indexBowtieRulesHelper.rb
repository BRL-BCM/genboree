...
module BRL ; module Genboree ; module Tools
  class IndexBowtieRulesHelper < WorkbenchRulesHelper
    ...
    def warningsExist?(wbJobEntity)
      ...
      if(wbJobEntity.context['warningsConfirmed'])
        ...
        @outputIndexFile = "#{@indexBaseName}_bowtie2.tar.gz"
        filePath = "#{output}/file/indexFiles/bowtie/#{@indexBaseName}/#{CGI.escape(@outputIndexFile)}?"

        ## Look for index in user db or repository db
        if(@fileApiHelper.exists?(filePath, @hostAuthMap))
          ## Index found in user db
          errorMsg = "Bowtie2 Index with same name <b>#{@outputIndexFile}</b> is available in user DB <b>#{CGI.unescape(output)} </b>. Do you want to replace this index? "
          warnings = true
        else
          ...
          @roiDirs.each { |dirName|
            apiCaller = ApiCaller.new(indexUri.host, "#{rsrcPath}/files/indexFiles/bowtie/#{CGI.escape(dirName)}/#{CGI.escape(@outputIndexFile)}", @hostAuthMap)
            apiCaller.get()
            if(apiCaller.succeeded?)
              ## The index is available in common database 
              errorMsg = "Bowtie2 Index with name <b>#{@outputIndexFile}</b> is available in repository DB <b>#{CGI.unescape(@roiRepoDb)}</b>. You may consider using the existing index or create a new index in your database with the same name.<br> Do you want to continue building the index?"
              warnings = true
