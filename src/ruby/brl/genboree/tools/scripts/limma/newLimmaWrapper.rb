...
require 'brl/genboree/rest/helpers/folderApiUriHelper'
...
module BRL ; module Genboree; module Tools
  class WrapperUserLimma < ToolWrapper
    ...
    def mergeMatrixFiles(matrixFileList)
      matrixFileList.each_index{|ii|
        downloadFile(matrixFileList[ii],"#{@outputDir}/matrix#{ii}.tmp")
        expander=BRL::Genboree::Helpers::Expander.new("#{@outputDir}/matrix#{ii}.tmp")
        expander.extract('text')
        FileUtils.mv(expander.uncompressedFileName,"#{@outputDir}/matrix#{ii}.txt")
        FileUtils.rmdir(expander.tmpDir)
        FileUtils.rm("#{@outputDir}/matrix#{ii}.tmp")
      }
      matrixFileCount = matrixFileList.length

    end

    def retrieveMatrixFilefromFolder(matrixFileInput)
      resultFileName = "matrix.xls.zip"
      toolIdStr = "epigenomicSlice"
      toolStrAttributeName = "JobToolId"
      jobIdAttributeName = "CreatedByJobName"
      @apiCaller.setHost(@fileHelper.extractHost(matrixFileInput))
      gbKey=""
      if(@dbHelper.extractGbKey(input)) then gbKey = "?gbKey=#{@dbHelper.extractGbKey(matrixFileInput)}" end
      path = @folderHelper.extractPath(matrixFileInput)
      path << gbKey
      @apiCaller.setRsrcPath(path)
      @apiCaller.get
      if !(@apiCaller.succeeded?) then
        $stderr.puts @apiCaller.respBody().inspect
        @errMsg = "Unable to obtain listing of contents of #{matrixFileInput}"
        raise @errMsg
      else
        @apiCaller.parseRespBody()
        resultFileSuffix = nil
        found = false
        @apiCaller.apiDataObj.each{|aa|
          if(aa["label"] =~ /#{resultFileName}$/) then
            resultFileSuffix = aa["label"]
            found = true
            $stderr.puts "Found #{aa["label"]}"
          end
        }
        if(!found) then
          @errMsg = "The folder #{matrixFileInput} does not seem to contain a file named #{resultFileName}"
          raise @errMsg
        else
          matrixFilePath = "#{@dbHelper.extractPath(matrixFileInput)}/file/#{resultFileSuffix}"
          attrPath = "#{matrixFilePath}/attribute/{attr}/value"
          attrPath << gbKey
          @apiCaller.setRsrcPath(attrPath)
          @apiCaller.get({:attr => toolStrAttributeName})
          if !(@apiCaller.succeeded?) then
            $stderr.puts @apiCaller.respBody().inspect
            @errMsg = "Unable to obtain value of attribute #{toolStrAttributeName} of file #{matrixFilePath}"
            raise @errMsg
          else
            @apiCaller.parseRespBody()
            if (@apiCaller.apiDataObj["text"] != toolIdStr) then
              @errMsg = "The value of attribute #{toolStrAttributeName}:#{@apiCaller.apiDataObj["text"]} of file #{matrixFilePath} does not match the required value #{toolIdStr}"
              raise @errMsg
            else
              matrixFileURI = "http://#{@dbHelper.extractHost(matrixFileInput)}#{matrixFilePath}"
            end
          end
        end
      end
    return matrixFileURI
    end


    def processJobConf()
      @inputs      = @jobConf["inputs"]
      @matrixFileInput = @inputs[0]
      ...
      @folderHelper = BRL::Genboree::REST::Helpers::FolderApiUriHelper.new()
    ...
    def run
      if (@matrixFileInput =~ BRL::Genboree::REST::Helpers::FolderApiUriHelper::NAME_EXTRACTOR_REGEXP) then
      end
