...
class AnalyzeReads
  ...
  attr_accessor :nodes, :kspan, :refGenome, :basename, :lffFile, :readName, :adapterSeq
  ...
  def initialize(readsFile, outputDir, targetGenome, kWeight, refGenome, lffFile, maxMapping, scratch, diagonals, gap, topPercent, ignorePercent, nodes, kspan, usableReads, adapterSeq)
  ...
  def removeAdaptors
    puts @adapterSeq
    command = "prepareSmallRNA.fastq.rb -r #{@readsFile} -o #{@outputDir} -u #{@usableReadsCheck} -a #{@adapterSeq}> #{@outputDir}/log.prepareSmallRNA 2>&1"
    system(command)
    $stderr.puts "prepareSmallRNA command = #{command}"
    system("grep -v '[+|h]' #{@outputDir}/#{@baseName}.output.tags |sed 's/@/>/' > #{@outputDir}/#{@baseName}_WithoutAdaptors.fa")
    system("grep @ #{@outputDir}/#{@baseName}.output.tags | cut -d'_' -f3 |rubySumInput.rb > #{@outputDir}/UsableReads.txt")
    filereader = File.open("#{@outputDir}/UsableReads.txt")
  ...
  def analyze
    ...
    puts @usableReadsCheck
    if(@usableReadsCheck=='T')
      command = "accountForMappings.rb -p #{@outputDir}/#{@baseName}.pash3.0.Map.output.txt -o #{@outputDir} -r #{@outputDir}/#{@baseName}_WithoutAdaptors.fa -R #{@refGenome} -l #{@lffFile} -u #{@usableReads}"
      $stderr.puts "Running accountForMapping considering usable reads = #{command}"
    else
      command = "accountForMappings.rb -p #{@outputDir}/#{@baseName}.pash3.0.Map.output.txt -o #{@outputDir} -r #{@outputDir}/#{@baseName}_WithoutAdaptors.fa -R #{@refGenome} -l #{@lffFile}"
       $stderr.puts "Running accountForMapping NOT considering usable reads = #{command}"
    end
...
class RunScript
  ...
  def self.parseArgs()
    ...
    optsArray=[
      ...
      ['--adapterSeq' , '-a', GetoptLong::OPTIONAL_ARGUMENT],
  ...
  def self.performAnalyzeReads(optsHash)
    AnalyzeReads.new(optsHash['--readsFile'], optsHash['--outputDir'], optsHash['--targetGenome'], optsHash['--kWeight'], optsHash['--refGenome'], optsHash['--lffFile'], optsHash['--maxMapping'], optsHash['--scratch'], optsHash['--diagonals'], optsHash['--gap'], optsHash['--topPercent'], optsHash['--ignorePercent'], optsHash['--nodes'], optsHash['--kspan'], optsHash['--usableReads'], optsHash['--adapterSeq'])
  end
