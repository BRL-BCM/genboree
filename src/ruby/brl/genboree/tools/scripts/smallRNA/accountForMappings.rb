...
class AccountMappings
  ...
  def initialize(pashFile, outputDir, readsFile, refFile, lffFile, usableReads)
    ...
    @readsFile = readsFile
    @readsFileHttp = CGI.escape(@readsFile)
    @refFile = refFile
    @lffFile = lffFile
    @readName = File.basename(@readsFile)
   # @readName = @readName.split(".")
   # @readName = readName[0]
    #@readsFile = CGI.escape(@readsFile)
    @readNameHttp = CGI.escape(@readName)
    if(usableReads )
      @usableReads = usableReads.to_i
    else
      @usableReads = 'N'
    end
    ...
    if(outputDir)
      @outputDir = outputDir
      @outputDirHttp = CGI.escape(@outputDir)
    else
      @outputDir = Dir.pwd
    end
    ...
    $stdout.puts "Reads Offset File Created"
    
    getLengths() # Wrapper for getMappingLengths.exe
    $stdout.puts "Mappings Lengths Calculated"
    
    intersect() # Wrapper for mappingsOntoLff.exe
    $stdout.puts "Intersection done"
    
    getExcelSheets() # For creating excel sheets
    $stdout.puts "Excel workbook created"
  ...
  def createReadsOffsetFile
    command = "makeReadsOffset.rb -r #{@readsFileHttp} -o #{@outputDirHttp}/#{@readNameHttp}.off > #{@outputDir}/log.readsOffset.#{@readName} 2>&1"
    system(command)
    $stdout.puts "makeReadsOffset command = #{command}"
  ...
  def getLengths
    ...
    system(command)
    $stdout.puts "getMappingLengths command = #{command}"
...
# collect lff class:type:subtype
    ...
    puts command
    system(command)
    $stdout.puts "mappingsOntoLff command = #{command}"
  end
  ...
  def getExcelSheets
    ...
    grepCommand =  "grep 'miRNA_Cluster\\|RNA_miRNA' #{@outputDir}/#{@readName}.trackCoverage.lff > #{@outputDir}/miRNA.lff"
    $stdout.puts "grep command #{grepCommand}"
    system(grepCommand)
    ...
    system("grep miRNA_Precursors #{@outputDir}/#{@readName}.trackCoverage.lff > #{@outputDir}/precursor.lff")
    system("grep miRNA_Mature #{@outputDir}/#{@readName}.trackCoverage.lff > #{@outputDir}/mature.lff")
    ...
    sheet8 = book.create_worksheet
    sheet9 = book.create_worksheet
    ...
    sheet7.name = "scaRNAs"
    sheet8.name = "miRNA_Precursors"
    sheet9.name = "miRNA_Mature"
    ...
    precursorRead = IO.popen("sort -k 10,10nr #{@outputDir}/precursor.lff" ,"r")
    matureRead = IO.popen("sort -k 10,10nr #{@outputDir}/mature.lff")
	...
	lineCount += 1
      ...
      if(@usableReads =='N')
	sheet1[0,3] = CGI.unescape(@readName); sheet1[1,3] = "Total reads"; sheet1[2,3] = "Count"; sheet1[2,4] = "%"
      else
	sheet1[0,3] = CGI.unescape(@readName); sheet1[1,3] = "Usable reads"; sheet1[2,3] = "Count"; sheet1[2,4] = "%"
      end
	...
	rowCount += 1	
      ...
      tempArray = Array.new()
      ii = 0
      precursorRead.each { |line|

	line = line.chomp.split(/\t/)
	sumPrecursor = sumPrecursor + line[9].to_i
	sheet8.row(rowCount).insert 0, line[1], line[9].to_i
	tempArray[ii] = line[9].to_i
        rowCount += 1
        ii += 1
	...
	sheet8.row(rowCount).insert 2, val
	rowCount += 1	
      ...
      tempArray = Array.new()
      ii = 0
      matureRead.each { |line|

	line = line.chomp.split(/\t/)
	sumMature = sumMature + line[9].to_i
	sheet9.row(rowCount).insert 0, line[1], line[9].to_i
	tempArray[ii] = line[9].to_i
        rowCount += 1
        ii += 1
	...
	sheet9.row(rowCount).insert 2, val
	rowCount += 1	
      ...
      sheet8[1,0] = "Name"; sheet8[0,1] = "#{@readName}.miRNA_Precursor"; sheet8[1,1] = "Count"; sheet8[1,2] = "Percentage"
      sheet9[1,0] = "Name"; sheet9[0,1] = "#{@readName}.miRNA_Mature"; sheet9[1,1] = "Count"; sheet9[1,2] = "Percentage"
