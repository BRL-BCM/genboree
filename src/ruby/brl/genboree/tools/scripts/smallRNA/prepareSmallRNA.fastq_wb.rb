...
class TrimSmallRNAs
  ...
  def setParameters()
    ...
    @outputFile = @outputFile.gsub(' ','%20')
    @readsFile = @readsFile.gsub(' ','%20')
   
    system("mkdir #{@outputFile}")
    @usableReads = @optsHash['--usableReads']
      ...
      @adapterTag = @optsHash['--adaptorSequences']
  ...
  def TrimSmallRNAs.processArguments()
		...
		optsArray =	[					['--readsFile',       '-r', GetoptLong::REQUIRED_ARGUMENT],
									...
									['--minHomoPolymer', '-H', GetoptLong::OPTIONAL_ARGUMENT],
									['--minReadOccurance','-R', GetoptLong::OPTIONAL_ARGUMENT],
									['--adaptorSequences','-a',GetoptLong::OPTIONAL_ARGUMENT],
...
COMMAND LINE ARGUMENTS:
  ...
  --maxReadLength      | -M  => (Optional) by default 30. Max length of read
  --minReadLength      | -m  => (Optional) by default 10. Min length of read
  --minHomoPolymer     | -H  => (Optional) by default 9. Min length of trailing homo polymer length
  --minReadOccurance   | -R => (Optional) by default 5. Min occurance of reads
  --adaptorSequences   | -a  => (Optional) adaptor sequence
