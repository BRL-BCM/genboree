...
class Heat
  ...
  def makeHeatMap()
    ...
    # Changed to use the 'better' heatmap produced as a result of the gclus package. In this case the hclust object is computed first, reordered and it's dendrograms fed to the heatmap.
    # This has the added benefit of easily creating newick files matching the dendrograms
  ...
  mat=data.matrix(x)
  hr<-hclust(dist(mat))
  hr<-reorder.hclust(hr,dist(mat))
  rowv=as.dendrogram(hr)
  hc<-hclust(dist(t(mat)))
  hc<-reorder.hclust(hc,dist(t(mat)))
  colv=as.dendrogram(hc)
  write(hc2Newick(hr),file=\"row.newick\")
  write(hc2Newick(hc),file=\"col.newick\")
