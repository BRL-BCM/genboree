...
module BRL ; module Script
  class DriverLimmaSignalComparison < ScriptDriver
    ...
    COMMAND_LINE_ARGS = {
      ...
      "--output"     =>  [ :REQUIRED_ARGUMENT, "-o", "output d/b" ],
      "--jobId"     =>  [ :REQUIRED_ARGUMENT, "-j", "jobId" ],
    ...
    def runBuildMatrixTool()
      ...
      cmd ="epigenomicStressSliceTool.rb "
      cmd << " -t '#{@scratch}/tmpFile.txt' -S #{@scratch}  -A #{@aggF} -a #{@apiDBRC} -r '#{CGI.escape(@roi)}' -u #{@userId} -R #{@removeNoData} -o #{CGI.escape(@output)} -j #{CGI.escape(@jobId)}"
