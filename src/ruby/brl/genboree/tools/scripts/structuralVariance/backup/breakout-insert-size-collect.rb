...
class BreakoutCollectInsert
  ...
  def setParameters()
	...
	def submitJob(inputIndex, node)
		...
		command << " -M 100000 -T #{@fileType} -F #{@forwardSuffix} -R #{@reverseSuffix} -S #{@orientation} >> #{File.dirname(@outputHistogram)}/IC.log 2>>#{File.dirname(@outputHistogram)}/IC.errorlog"
