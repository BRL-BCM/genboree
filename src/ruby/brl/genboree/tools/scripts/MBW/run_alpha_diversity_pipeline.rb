...
class AlphaDiversity
  ...
  #usage
  def AlphaDiversity.usage()
    if ARGV.size != 3 || ARGV[0] =~ /-h/
      $stderr.puts "\n============USAGE============"
      $stderr.puts "runAlphaDiversity.rb <QIIME output folder> <output folder> <configuration settings file>\n\n"
      exit 1 
    end
  end
  ...
  def initialize(qiimeOutputFolder, outputDirectory, settingsFile)
    @QIIMEfolder = fep(qiimeOutputFolder) + "/"
    @communityFile = @QIIMEfolder + "otu_table.txt"
    @environmentFile = @QIIMEfolder + "mapping.txt"
    @outputDirectory = fep(outputDirectory) + "/alphadiversity/"
    @settingsFile = fep(settingsFile)
  ...
  def runSpeciesAccumulationCurvesRichness()
    ...
    cmds << "`accumcompMOD` <-"
    cmds << "function(x,y=\"\",factor,scale=\"\",method=\"accMethod\",permutations=#{@settings.instance_variable_get(:@permutations)},conditioned=T,gamma=\"Boot\",plotit=T,labelit=T,legend=T,rainbow=T,xlim=c(1,max),ylim=c(0,rich),type=\"p\",xlab=\"#{@sampleHeader}\",ylab=\"species richness\",...) {"
    ...
    cmds << "        }else {"
    cmds << "            colors <- c(#{@settings.instance_variable_get(:@colors)})"
    cmds << "            palette(colors)"
    cmds << "            accumplotMOD(result1,method=method, factor=factor,addit=addit,xlab=xlab,ylab=ylab,xlim=xlim,ylim=ylim,labels=labels,col=i,pch=i,type=type,...)"
    cmds << "        }"
    cmds << "        }"
    cmds << "    }"
    cmds << "    #if (rainbow==T && legend==T) {legend(locator(1),legend=levels,pch=c(1:m)+#{@settings.instance_variable_get(:@legendMarkerSizeMod)},col=c(1:m))}"
    cmds << "    #if (rainbow==F && legend==T) {legend(locator(1),legend=levels,pch=c(1:m)+#{@settings.instance_variable_get(:@legendMarkerSizeMod)})}"
    cmds << "    #http://astrostatistics.psu.edu/datasets/R/html/graphics/html/legend.html"
    cmds << "    legend(\"#{@settings.instance_variable_get(:@legendPosition)}\",legend=levels,pch=c(1:m)+#{@settings.instance_variable_get(:@legendMarkerSizeMod)},col=c(1:m))    "
    cmds << "    }"
    ...
    cmds << "    if (xpos > 200){"
    cmds << "      xpos = xpos - #{@settings.instance_variable_get(:@richnessOffset2)}"
    cmds << "    }"
    ...
    @metaArray.each{ |metaVal|
      ...
      accumulationMethods.each{ |accMethod|
        ...
        cmds << "pdf(file= \"#{outputFile}\", height=#{@settings.instance_variable_get(:@height)}, width=#{@settings.instance_variable_get(:@width)})"
        ...
        cmds << "  permutations=#{@settings.instance_variable_get(:@permutations)}, legend=#{@settings.instance_variable_get(:@legendBoolChar)}, rainbow=#{@settings.instance_variable_get(:@rainbow)}, ci=2, ci.type='bar', cex=1, "
  ...
  def runRankAbundance()
    ...
    @metaArray.each{ |metaVal|
      ...
      rankAbunMethods.each{ |rankAbunMethod|
        ...
        cmds << "  factor='#{metaVal}', scale='#{rankAbunMethod}', legend=#{@settings.instance_variable_get(:@legendBoolChar)}, rainbow=#{@settings.instance_variable_get(:@rainbow)})"
        cmds << "dev.off()"
      }    
    }   
  ...
  def runRenyiProfile()
    ...
    cmds << "            }"
    cmds << "        }"
    cmds << "        if (legend==T && n<26) {legend(\"#{@settings.instance_variable_get(:@legendPosition)}\",legend=rownames(x),pch=c(1:n)+#{@settings.instance_variable_get(:@legendMarkerSizeMod)},col=c(1:n))}"
    cmds << "        if (legend==T && n>25) {legend(\"#{@settings.instance_variable_get(:@legendPosition)}\",legend=rownames(x),pch=rep(19,n),col=c(1:n))}"
    cmds << "    }else{"
    cmds << "        colors <- c(#{@settings.instance_variable_get(:@colors)})"
    cmds << "        palette(colors)"
    cmds << "        for (i in 1:n) {"
    ...
    cmds << "            if (n>25) {points(c(1:p),x[i,],pch=19,col=i,cex=cex,type=\"o\")}"
    cmds << "            if (labelit==T) {"
    cmds << "                text(1,x[i,1],labels=rownames(x)[i],pos=2,col=i,cex=cex, offset=#{@settings.instance_variable_get(:@renyiOffset)})"
    cmds << "                #text(p,x[i,p],labels=rownames(x)[i],pos=4,col=i,cex=cex)"
    cmds << "            }"
    cmds << "        }"
    cmds << "        #if (legend==T) {legend(\"#{@settings.instance_variable_get(:@legendPosition)}\",legend=rownames(x),pch=c(1:n)+#{@settings.instance_variable_get(:@legendMarkerSizeMod)})}"
    cmds << "        if (legend==T && n<26) {legend(\"#{@settings.instance_variable_get(:@legendPosition)}\",legend=rownames(x),pch=c(1:n)+#{@settings.instance_variable_get(:@legendMarkerSizeMod)},col=c(1:n))}"
    cmds << "        if (legend==T && n>25) {legend(\"#{@settings.instance_variable_get(:@legendPosition)}\",legend=rownames(x),pch=rep(19,n),col=c(1:n))}"
    cmds << "    }"
    cmds << "    palette(\"default\")"
  ...
  y=EnvironmentalDataset, factor='#{metaVal}', scales=c(#{@settings.instance_variable_get(:@renyiScale)}), permutations=#{@settings.instance_variable_get(:@permutations)}, legend=#{@settings.instance_variable_get(:@legendBoolChar)})"
  ...
  def convertPDFtoPNG(pdfList)
    pdfList.each{ |pdfFile|
      #puts convertCmd = "convert -density #{@settings.instance_variable_get(:@pngDensity)} #{pdfFile} #{pdfFile.gsub(/pdf/, "PNG")}"
      #`#{convertCmd}`
    }
...
AlphaDiversity.usage()

#initialize input data
alphaDiv = AlphaDiversity.new(*ARGV)
