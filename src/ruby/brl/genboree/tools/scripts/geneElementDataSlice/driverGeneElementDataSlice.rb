...
require 'brl/genboree/rest/helpers/trackEntityListApiUriHelper.rb'
include BRL::Genboree::REST

# Write sub-class of BRL::Script::ScriptDriver
module BRL ; module Script
  class DriverGeneElementDataSlice < ScriptDriver
    # ------------------------------------------------------------------
    # SUB-CLASS INTERFACE
    ...
    COMMAND_LINE_ARGS = {
      ...
      "--geneList"    =>  [ :OPTIONAL_ARGUMENT, "-r", "gene List" ],
    ...
    DESC_AND_EXAMPLES = {
      :description => "Driver to run tool, which generates matrix on epigenome data for specific genes (custom made ROI)",
    ...
    def run()
      ...
      if(File.exists?("#{@scratch}/matrix")  and  File.exists?("#{@scratch}/logs"))
    ...
    def buildHashofVectorEntity()
      ...
      @trackSets.each {|trackSet|
        ...
        if(trackSet =~ BRL::Genboree::REST::Helpers::TrackEntityListApiUriHelper::NAME_EXTRACTOR_REGEXP)
      ...
      ##Download gene list
      tempFile1 = File.open("#{@scratch}/geneList.txt","w+")
      encodedGeneList = @geneList
      tmpPath = "#{@path2}/data?"
      tmpPath << "gbKey=#{@dbApiHelper.extractGbKey(encodedGeneList)}" if(@dbApiHelper.extractGbKey(encodedGeneList))
      ...
      if apicaller.succeeded?
        $stderr.debugPuts(__FILE__, __method__, "Build Matrix Tool", "GeneList downloaded successfully")
      else
        $stderr.debugPuts(__FILE__, __method__, "Build Matrix Tool- ERROR", apicaller.parseRespBody().inspect)
        exitCode = apicaller.apiStatusObj['statusCode']
      end
    ...
    def runBuildMatrixTool()
      ...
      cmd ="geneElementDataSliceTool.rb "
      cmd << " -t '#{@scratch}/tmpFile.txt' -S #{@scratch}  -a #{@apiDBRC} -r '#{@scratch}/geneList.txt' -R #{@removeNoData} -u #{@userId} "
...
if($0 and File.exist?($0) and BRL::Script::runningThisFile?($0, __FILE__, true))
  ...
  BRL::Script::main(BRL::Script::DriverGeneElementDataSlice)
end
