...
module BRL ; module Genboree; module Tools
  class WrapperEpigenomicSlice < ToolWrapper
    ...
    def run()
      ...
      exitStatus = EXIT_OK
      @errMsg = ""
      cmd ="driverGeneElementDataSlice.rb "
      @entityBuffer = ""
      fileInput = ""
      @inputs.each{|input|
        if(input =~ BRL::Genboree::REST::Helpers::FileApiUriHelper::NAME_EXTRACTOR_REGEXP) then
          fileInput = CGI.escape(input)
        else
          @entityBuffer << "#{CGI.escape(input)},"
        end
        }
       if(@removeNoData == "on")
        @removeNoData = "true"
      ...
      @entityBuffer.chomp!(",")
      cmd << " -t '#{@entityBuffer}'"
      cmd << " -S #{@scratch} -r #{fileInput} "
      cmd << " -a #{@apiDBRCkey} -R #{@removeNoData} -u #{@userId} "
     ...
     def uploadUsingAPI(jobName,fileName,filePath)
      @exitCode = 0
      restPath = @outPath
      path = restPath +"/file/GeneElementDataSlice/#{CGI.escape(jobName)}/#{fileName}/data"
    ...
    def prepSuccessEmail
      ...
      emailObject.resultFileLocations = "http://#{@outHost}/java-bin/apiCaller.jsp?fileDownload=true&promptForLogin=true&rsrcPath=#{CGI.escape("#{@outPath}/file/GeneElementDataSlice/#{CGI.escape(@analysis)}/matrix.xls.gz/data")}"
