...
module BRL; module Genboree; module Tools; module Scripts
  class MrfAnnotationCoverageWrapper < BRL::Genboree::Tools::ToolWrapper
    VERSION = "1.0"
    COMMAND_LINE_ARGS = { }
    DESC_AND_EXAMPLES = {
      :description  => "This is the wrapper for running 'MrfAnnotationcoverage' module from the RSeqTools package.
    ...
    def processJobConf()
      begin
        # Options to run mrfAnnotationCoverage
        @mrfFile = @inputs[0]
        @coverageFactor = @settings['coverageFactor']
        @readsToSample = @settings['readsToSample']
        @analysisName = @settings['analysisName']
        
        @scratchDir = @context['scratchDir']
        @outFile = "#{@scratchDir}/#{CGI.escape(@analysisName)}_coverage.txt"
        ...
        if(!@genomeVersion.nil?)
          ...
          if(@geneAnnoIndexBaseName.nil?)
            @errUserMsg = "The gene annotations for genome: #{@genomeVersion} could not be found since this annotation is not supported currently.\nPlease contact the Genboree Administrator for adding support for this gene annotation. "
            raise @errUserMsg
          end
        end
    ...
    def run()
      begin
        # Run the tool. 
        @errFile = "#{@scratchDir}/mrfAnnotationCoverage.err"
        @numTotalReads = `wc -l #{@mrfFile}`.strip.to_i
        if(@numTotalReads > @readsToSample)
          @numReadsToSample = @readsToSample
        else
          @numReadsToSample = @numTotalReads
        end
        @count = 1
        while(@numReadsToSample <= @numTotalReads) do
          command = "cat #{@mrfFile} | mrfAnnotationCoverage #{@knownGeneCompositeModel} #{@numTotalReads} #{@numReadsToSample} #{@coverageFactor}"
          if(@count == 1)
            command << " >> #{@outFile} 2> #{@errFile}"
          else
            command << " | grep -v fractionDetected >> #{@outFile} 2> #{@errFile}"
          end
          $stderr.debugPuts(__FILE__, __method__, "STATUS", "Launching command: #{command}")
          exitStatus = system(command)
          if(!exitStatus)
            @errUserMsg = "MrfAnnotationCoverage failed to run"
    ...
    def prepSuccessEmail()
      additionalInfo = "MrfAnnotationCoverage ran successfully."
...
if($0 and File.exist?($0) and BRL::Script::runningThisFile?($0, __FILE__, true))
  ...
  BRL::Genboree::Tools::main(BRL::Genboree::Tools::Scripts::MrfAnnotationCoverageWrapper)
end
