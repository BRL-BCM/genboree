    ...
    otuRead.each{ |otuLine|
      ...
      if hOTU[otuNum] != 0
        ...
        groupArr.each_index{|ii|
          tmpCount = 0
            rWrite.print "arr_#{ii}_#{otuNum}=c("
            groupArr[ii].each{|g0|
              if g0.to_i == 0
        ...
        if(!hGmedians.has_key?(otuNum)) then hGmedians[otuNum] = [] end
        #store median
        groupArrCon.each{|gg|
          hGmedians[otuNum] << calc_median(gg)
          }            
        groupArr.each{|gg| gsize << gg.length}
        hMedianDifference[otuNum] = hGmedians[otuNum]
      end
      otuCount += 1
    }
#
#puts otuCount
    ...
    otuOrderReverseArr.each{ |sortedOrderedOTU|
      ...
      next if hGmedians[sortedOrderedOTU].uniq == [-6.0]
    ...
    otuOrderReverseArr.each{ |sortedOrderedOTU|
     ...
     # puts "#{sortedOrderedOTU}:#{hG0medians[sortedOrderedOTU]}:#{hG1medians[sortedOrderedOTU]}"
      ...
      arr = [uniqueVals.map{|uu| "\"#{fullName}-#{uu}\""}.join(","),uniqueVals.map{|uu| "expression(bold(\"#{fullName}-#{uu}\"))"}.join(",")]
      if finalFilteredCount % 2 == 0
        rWrite.print arr[fullNameCount % 2]
      else
        rWrite.print arr.reverse[fullNameCount % 2]
      end
    ...
    otuOrderReverseArr.each{ |sortedOrderedOTU|
      ...
      rWrite.print groupArr.each_index.map{|ii| "arr_#{ii}_#{sortedOrderedOTU}"}.join(",")      
      validOTUsCount += 1
    }  
#
#puts validOTUsCount
colorArr = ["red","blue","green","orange","brown","black","cyan","darkblue","lightblue","purple","yellow","lime","fuchsia","white","silver","gray","maroon","olive"]
    ...
    colorString = colorArr[0 .. uniqueVals.length-1].map{|xx| "\"#{xx}\""}.join(",")
    colorCode = "col=c(#{colorString})"
    ...
    rWrite.puts "legend(par(\"usr\")[1],par(\"usr\")[3], c(#{uniqueVals.map{|uu| "\"#{uu}\""}.join(",")}), fill = c(#{colorString}),lty=2,xjust=1.1,yjust=1.1,cex=#{yLabelSize})"
