...
class UploadHelper
  ...
  attr_accessor :projectUri
  
  def uploadFile(pathFromBase,diskFilePath)
    rsrcPath = "#{@dbApiHelper.extractPath(@baseUri)}/#{pathFromBase}/data"
    rsrcPath << "?gbKey=#{@dbApiHelper.extractGbKey(@baseUri)}" if(@dbApiHelper.extractGbKey(@baseUri))
    @apiCaller.setRsrcPath(rsrcPath)
    @apiCaller.put(File.open(diskFilePath,"r"))
    if @apiCaller.succeeded?
      $stderr.debugPuts(__FILE__,__method__,"STATUS","Successfully uploaded #{diskFilePath}")
    else
      msg = "Upload of #{diskFilePath} did not succeed\nrsrcPath=#{@apiCaller.rsrcPath}\nrespBody=#{@apiCaller.respBody.inspect}"
      $stderr.debugPuts(__FILE__,__method__,"ERROR",msg)
      raise msg
    end
  end
  
  def rsyncFiles(srcDir,destDir)
    #cmd = "rsync -avz #{srcDir} #{ApiCaller.getDomainAlias(@dbApiHelper.extractHost(@baseUri))}:/usr/local/brl/local/apache/htdocs/projects/#{destDir}"
    #$stderr.debugPuts(__FILE__,__method__,"STATUS","rsync command:#{cmd}")
    #system(cmd)
    Dir.chdir(srcDir)
    jobDir = srcDir.split("/").last
    `tar -cf #{jobDir}.tar *; gzip #{jobDir}.tar`
    uriObj = URI.parse(@projectUri)
    apiCaller = WrapperApiCaller.new(uriObj.host, "#{uriObj.path}/#{destDir}/file/#{jobDir}/#{jobDir}.tar.gz?extract=true", @userId)
    apiCaller.put({}, File.open("#{jobDir}.tar.gz"))
  end
  
  def initialize(baseUri, userId)
    @dbApiHelper = BRL::Genboree::REST::Helpers::DatabaseApiUriHelper.new
    
    $stderr.debugPuts(__FILE__,__method__,"DEBUG",baseUri)
    @baseUri = baseUri.gsub(/\?$/,"")
    @apiCaller =  WrapperApiCaller.new(@dbApiHelper.extractHost(@baseUri),"", userId)
  end
...
#
#
#  # Copies dir structure of the ML job to the target project
#  # [+returns+] nil
#  def importMLFiles()
#    Dir.chdir("#{@inputDir}")
#    path = @inputDir.split("/")
...
#                            "updateText" => "#{@userFirstName.capitalize} #{@userLastName.capitalize} ran a Machine Learning job (#{@jobName}) and the results are available at the link below.
#                            <ul>
#                              <li><b>Study Name</b>: #{@studyName}</li>
#                              <li><b>Job Name</b>: #{@jobName}</li>
#                              <li><a href=\"/projects/#{CGI.escape(@projectName)}/genb%5E%5EadditionalPages/#{CGI.escape(@jobDir)}/mlIndex.html\">Link to result plots</a></li>
#                            </ul>"
#                          }
#                        )
...
#    plotsIndexWriter = File.open("./mlIndex.html", "w")
#    plotBuff = "<html>
...
#                            <a href=\"/projects/#{CGI.escape(@projectName)}/genb%5E%5EadditionalPages/#{CGI.escape(@jobDir)}/RF_Boruta/#{dir}/graph/#{file}\">#{file.gsub(".PNG", "").capitalize}</a>
#                          </td>
#                        </tr>
#                      "
#              else
...
#
#  Description: This script is used for copying the Machine Learning files from a specified input directory to a target project.
...
#  end
#
#  def self.performImportMLFiles(optsHash)
#    rdpObj = ImportMLFiles.new(optsHash)
#  end
#
#end
#
#optsHash = RunScript.parseArgs()
#RunScript.performImportMLFiles(optsHash)
