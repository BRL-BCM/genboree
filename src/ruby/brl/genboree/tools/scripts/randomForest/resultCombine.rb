...
uniqueCount = uniqueMetaNames.length
...
listOut.print "RDP Taxonomy\tOTU#"
prefixes=namesArr[0 .. uniqueCount-1].map{|nn| nn.split(/\-\d+$/)[0]}

prefixes.each{|prefix|
  listOut.print "\t#{prefix}-min\t#{prefix}-q1\t#{prefix}-median\t#{prefix}-q3\t#{prefix}-max"
}
listOut.print "\tU\tZ"
prefixes.each{|prefix|
  listOut.print "\t#{prefix}-Uscore"
}
prefixes.each{|prefix|
  listOut.print "\t#{prefix}-h1\t#{prefix}-h2\t#{prefix}-h3\t#{prefix}-botWhisk\t#{prefix}-topWhisk"
}
listOut.print "\tDirectional_Change\tBoruta"
listOut.puts
...
taxArr.each{ |line|
  ...
  listOut.print line
  from = loopCount*uniqueCount
  to = from+uniqueCount-1
  namesArr[from .. to].each_index{|count|
  listOut.print "\t#{minArr[count].to_f}\t#{q1Arr[count].to_f}\t#{medianArr[count].to_f}\t#{q3Arr[count].to_f}\t#{maxArr[count].to_f}"
  }
  listOut.print "\t#{uArr[loopCount]}\t#{zArr[loopCount]}"
  namesArr[from .. to].each_index{|count|
    listOut.print "\t#{sampUarr[count].to_f}"
  }
  namesArr[from .. to].each_index{|count|
    listOut.print "\t#{h1Arr[count].to_f}\t#{h2Arr[count].to_f}\t#{h3Arr[count].to_f}\t#{botWhiskArr[count].to_f}\t#{topWhiskArr[count].to_f}"
  }
listOut.print "\tUP in #{namesArr[sampUarr.find_index(sampUarr.max)].split("-")[0]}"
listOut.puts "\t#{boruta}"
loopCount += 1
}
listOut.close()
...
borutaColumn = (uniqueCount*11) + 6
uColumn = (uniqueCount*5)+3
zColumn = uColumn+1
sortCMD = "sort -t $'\t' -k #{borutaColumn},#{borutaColumn} -k #{uColumn},#{uColumn}n -k #{zColumn},#{zColumn}n #{otuListOut} > #{otuListOutSorted}"
