...
module BRL ; module Genboree ; module Tools
  class BigFilesRulesHelper < WorkbenchRulesHelper
      ...
      if(rulesSatisfied)
        ...
        if(!@dbApiHelper.allAccessibleByUser?(inputs, userId, CAN_WRITE_CODES))
          ...
          if(rulesSatisfied)
            ...
            inputs.each { |input|
              fullDbUri = @dbApiHelper.extractPureUri(input)
              if(@trkApiHelper.extractName(input)) # For tracks
                trkName = @trkApiHelper.extractName(input)
                @trkUriHash["#{fullDbUri}/trk/#{CGI.escape(trkName)}"] = nil
    ...
    def warningsExist?(wbJobEntity)
      ...
      if(wbJobEntity.context['warningsConfirmed'])
        ...
        warningsExist = false
        # Generate warning if even a single track is non high density
        ...
        settings.each_key { |key|
          if(key =~ /^#{baseWidget}/)
            if(settings[key])
              trkUri = key.split("|")[1]
              if(!@trkApiHelper.isHdhv?(trkUri, @hostAuthMap))
                wbJobEntity.context['wbErrorMsg'] = "You have non high density tracks (not uploaded via wig/bedGraph) in your selected list of tracks. The tool will fail if there are overlapping regions in your data since the UCSC bigwig program does does not support overlapping data."
                warningsExist = true
                break
              end
            end
