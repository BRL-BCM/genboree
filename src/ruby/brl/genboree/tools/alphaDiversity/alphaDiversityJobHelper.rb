...
module BRL ; module Genboree ; module Tools
  class AlphaDiversityJobHelper < WorkbenchJobHelper
    ...
    def initialize(toolIdStr, genbConf=nil, dbu=nil, *args)
      ...
      self.class.commandName = "alphaDiversityWrapper.rb"
    end
    ...
    def postCmds()
      ...
      group = @grpApiHelper.extractName(outputs[0]) # Group can be determined from both db and project Uri
      ...
      escApiDbFileUrl =CGI.escape("http://#{uri.host}/REST/v1/grp/#{CGI.escape(group)}/db/#{CGI.escape(db)}/file/MicrobiomeWorkBench/#{studyName}/AlphaDiversity/#{jobName}/jobFile.json/data?")
      return [
        "fileApiTransfer.rb #{@userId} ./jobFile.json #{escApiDbFileUrl}"
      ]
    end
    ...
    def cleanJobObj(workbenchJobObj)
      ...
      studyName = CGI.escape(workbenchJobObj.settings['studyName'])
      jobName = CGI.escape(workbenchJobObj.settings['jobName'])
      removeSingletons = workbenchJobObj.settings['removeSingletons']
      workbenchJobObj.settings['removeSingletons'] = removeSingletons ? true : false
