...
module BRL ; module Genboree ; module Tools
  class RenameSampleListRulesHelper < WorkbenchRulesHelper

    TOOL_ID = 'renameSampleList'
    ...
    def rulesSatisfied?(wbJobEntity, sectionsToSatisfy=[ :inputs, :outputs, :context, :settings ], toolIdStr=@toolIdStr)
      ...
      if(sectionsToSatisfy.include?(:settings))
        ...
        sampleEntityList = @sampleEntityListApiHelper.extractName(output)
        apiCaller = ApiCaller.new(targetHost, "#{targetRsrcPath}/samples/entityList/#{CGI.escape(newName)}?", @hostAuthMap)
        apiCaller.initInternalRequest(@rackEnv, @genbConf.machineNameAlias) if(@rackEnv)
        apiCaller.get()
        resp = JSON.parse(apiCaller.respBody)['data']
        if(!resp.empty?)
          wbJobEntity.context['wbErrorMsg'] = "ALREADY_EXISTS: A sample entity list with the name: '#{newName}' already exists in the target database. "
        else
          rulesSatisfied = true
        end
      end
