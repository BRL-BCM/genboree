...
retVal = resp['data']
ff = File.open("bedgraph_trackList.txt", "w")
retVal.each { |trk|
  ff.puts "#{trk['text'].inspect}"  
}
ff.close
spans = []
rois = ['Methylation%3A450']
roiMapHash =  {
      ...
      if(!File.exists?(fileName))
        ...
        if(adler32)
          ...
          if(func == 'avgByLength')
            ...
            `pbzip2 -p2 #{roi}/BedGraph/By%20#{func.capitalize}/#{CGI.escape(trk['text'])}.bedGraph`
          end
