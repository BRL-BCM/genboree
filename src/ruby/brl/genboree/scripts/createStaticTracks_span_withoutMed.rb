...
retVal = resp['data']
ff = File.open("span_without_med_trackList.txt", "w")
retVal.each { |trk|
  ff.puts "#{trk['text'].inspect}"  
}
ff.close
spans = [100000, 10000, 1000]
aggFuncs = ['avg', 'avgByLength', 'sum', 'count', 'max', 'min', 'stdev']
# Do spans:
spans.each { |span|
  aggFuncs.each { |func|
    retVal.each { |trk|
      ...
      if(!File.exists?(fileName))
        ...
        apiCaller = ApiCaller.new("valine.brl.bcmd.bcm.edu", "/REST/v1/grp/Epigenomics%20Roadmap%20Repository/db/Release%205%20Repository/trk/#{CGI.escape(trk['text'])}/annos?format=fwig&spanAggFunction=#{func}&emptyScoreValue=n%2Fa&addCRC32Line=true&span=#{span}", "paithank", "qppofa82")
