...
retVal = resp['data']
ff = File.open("prom_all_trackList.txt", "w")
retVal.each { |trk|
  ff.puts "#{trk['text'].inspect}"  
}
ff.close
spans = []
rois = ['Promoters%3AALL']
roiMapHash =  {
                "Promoters%3AALL" => "/REST/v1/grp/ROI%20Repository/db/ROI%20Repository%20-%20hg19/trk/Promoters%3AALL"
              }
