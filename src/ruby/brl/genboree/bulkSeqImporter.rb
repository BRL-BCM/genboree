...
class BulkSeqImportError < StandardError ; end

class BulkSeqImporter
  ...
  @@attributes = [  :verbose, :maxSeqFileSize, :fmetaInfo, :seqDir,:currSeqFileName,
                    ...
                    :altDbName, :seqDir, :pathDir, :dataDbName, :dbu, :dbrcFileName ]
  ...
  def initialize(optsHash)
    ...
    setParams(optsHash)
    if (@verbose)
        puts "dataDbName = #{@dataDbName}"
        puts "fastaFileName = #{@fastaFileName}"
        puts "maxSeqFileSize = #{@maxSeqFileSize.to_s}"
        puts "verbose = #{@verbose}"
        puts "altDbName = #{@altDbName}"
        puts "seqDir = #{@seqDir}"
        puts "pathDir = #{@pathDir}"
        exit(0)
    end
    init()
  ...
  def setParams(optsHash)
    ...
    @fastaFileName = optsHash['--fastaFileName']
    ...
    @altDbName = optsHash.key?('--altName') ? optsHash['--altName']:@dataDbName
    @seqDir = optsHash.key?('--useDir') ? "#{optsHash['--useDir']}/#{@altDbName}":"/usr/local/brl/data/genboree/ridSequences/#{@altDbName}"
    @pathDir = optsHash.key?('--pathDir') ? "#{optsHash['--pathDir']}/#{@altDbName}":"/usr/local/brl/data/genboree/ridSequences/#{@altDbName}"
    @dbrcFileName = nil
  end
  ...
  def getSeqDir()
    ...
    unless(!@fmetaInfo.nil? and @fmetaInfo.key?('RID_SEQUENCE_DIR') and !@fmetaInfo['RID_SEQUENCE_DIR'].empty?)
      ...
      @dbu.insertFmetaEntry('RID_SEQUENCE_DIR', @pathDir)
    #else
  ...
  # CLASS METHODS
  # ----------------------------------------------------------------------------
  def BulkSeqImporter.processArguments
    optsArray = [
                  ['--dataDbName', '-d', GetoptLong::REQUIRED_ARGUMENT],
                  ['--fastaFileName', '-f', GetoptLong::REQUIRED_ARGUMENT],
                  ['--useDir', '-u', GetoptLong::OPTIONAL_ARGUMENT],
                  ['--altName', '-a', GetoptLong::OPTIONAL_ARGUMENT],
                  ['--pathDir', '-p', GetoptLong::OPTIONAL_ARGUMENT],
    ...
    BulkSeqImporter.usage() if(optsHash.key?('--help') or !progOpts.getMissingOptions().empty?)
    return optsHash
  end

  def BulkSeqImporter.usage(msg='')
    puts "\n#{msg}\n" unless(msg.empty?)
    puts "
  ...
  COMMAND LINE ARGUMENTS:
    ...
    --fastaFileName      | -f  =>  Fasta file. One or more fasta recs.
    --useDir   |    -u  => [optional] directory to store the files
    --pathDir   |   -p   => [optional] directory path to store in the database field
    --altName   |   -a   => [optional] alternative name for directory
...
optsHash = BRL::Genboree::BulkSeqImporter.processArguments()
# Init deployment object, read properties file, set params
importer = BRL::Genboree::BulkSeqImporter.new(optsHash)
begin
  # Sequence upload
  importer.import()
rescue => err
