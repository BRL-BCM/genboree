...
module MultiPartDataExtractor
  ...
  # EM API Methods
  # ------------------------------------------------------------------
  # EM will call our constructor. We need some key context info to help us do our job.
  # @param [Hash] args The context Hash, passed through from EM.watch(). Must have:
  #   @:boundary@ - the multipart boundary (probably extracted from Content-Type header)
  #   @:formDataName@ - the name of the form data field you are interested in
  #   @:outFileName@ - full path to a file where data for the field will be saved; opened and closed by this object
  #   @:chunkSize@ - optional; generally if you want to try a larger chunk size...just make sure Ruby regexp search in
  #     String of such sizes will not take too long. String#index(regexp) is used extensively on the chunk.
  # @raise ArgumentError If you didn't provide required info above or your info is not sensible.
  def initialize(args)
  ...
  # ------------------------------------------------------------------
  # EM INTERFACE METHODS
  # ------------------------------------------------------------------

  # EM INTERFACE. Watches for when the IO becomes readable. For sockets it will do this as data becomes available.
  ...
  # @todo DANGER DANGER: if this class is adapted for use in an EM-based web server, you MUST REMOVE THE
  #   EM::stop_event_loop !!!!!
  def unbind()
    clean()
...
if(ARGV.size != 3)
  $stderr.puts "\nUSAGE:\n\n  multipartDataExtractor.rb {formField} {bodyFilePath} \"{boundary}\"\n\n"
  exit(134)
end

formField, filePath, boundary = ARGV[0], ARGV[1], ARGV[2]

# NOTE: chunkSize should not be this small and is optional anyway. Default is like 4K
args =
{
  :boundary => boundary,
  :formDataName => formField,
  :outFileName => "./#{File.basename(filePath)}.#{formField}.extracted",
  :chunkSize => 257
}

# EM will instantiate an extended version of our class above and give the object to us.
extractor = nil

# NOTE: don't need this EN.run() call in an environment that already has an EM loop (like thin)!!
EventMachine.run {
  # Get the IO to watch. Here it is the temp file nginx saved, which has the multipart request body!
  fh = File.open( filePath )
  # Arrange for EM to help us "watch" this File
  #  - 1st arg is the IO to watch
  #  - 2nd arg a Handler class that implements key EM API methods,
  #  - 3rd (or more) is extra arg[s] for your constructor [optional]
  extractor = EM.watch(fh, MultiPartDataExtractor, args)
  # Because it's a normal file that exists (not socket that will become readable), manually trigger our reading flow:
  extractor.notify_readable = true
}

$stderr.puts "Our multipart was #{extractor.multipartFound ? '' : 'NOT'} found. The user's name for the file was #{extractor.formFileName or 'N/A!'}"
