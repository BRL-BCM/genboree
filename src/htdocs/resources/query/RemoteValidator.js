	...
	init:function(field) {
		...
		Ext.apply(field, {
			...
			,validateRemote:function() {
                ...
                // Added by MFS, 06.21.10 - Check for loading indicator, if present show
                if(!Ext.isEmpty(this.rvOptions.loadingInd))
                {
                  var loadingInd = Ext.get(this.rvOptions.loadingInd) ;
                  if(loadingInd && !loadingInd.isVisible())
                  {
                    loadingInd.setVisibilityMode(Ext.Element.DISPLAY).show() ;
                  }
                }
			...
			,rvSuccess:function(response, options) {
                ...
                // Added by MFS, 06.21.10 - Check for loading indicator, if present hide
                if(!Ext.isEmpty(this.rvOptions.loadingInd))
                {
                  var loadingInd = Ext.get(this.rvOptions.loadingInd) ;
                  if(loadingInd && loadingInd.isVisible())
                  {
                    loadingInd.setVisibilityMode(Ext.Element.DISPLAY).hide() ;
                  }
                }
			...
			,filterRemoteValidation:function(e) {
				//if(!e.isNavKeyPress()) { // Edited by MFS (BNI) to not validate on special keys either...
		...
		field.on({
			render:{single:true, scope:field, fn:function() {
				...
				//this.el.on('blur', this.filterRemoteValidation, this); // Added by MFS (BNI), a bit redundant but needed...
			}}
		});
