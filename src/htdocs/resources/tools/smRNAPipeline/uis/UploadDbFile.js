...
UploadDbFile = {
  ...
  submitJob : function(form) {
    ...
    if (form.useLibrary.value == "uploadNewLibrary") {
  ...
  uploadCallback : function(request, success, response) {
    ...
    if(true == success)
    {
      ...
      try {
        ...
        {
          /**
           * Once file is uploaded the script appends the new file resource to a property in settings and then submit another request.
           **/
          //wbHash.get('inputs').push(respObj.data.refUri) ;
          //wbFormSettings.set('newSpikeInLibrary', respObj.data.refUri) ;
          $('newSpikeInLibrary').value = respObj.data.refUri ;
          submitToolJob(UploadDbFile.uploadForm) ;
