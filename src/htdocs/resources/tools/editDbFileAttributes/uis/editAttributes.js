 ...
 */
Ext.ux.data.GbFileAttrJsonReader = function(meta, recType) {
    // call parent constructor
    Ext.ux.data.GbFileAttrJsonReader.superclass.constructor.call(this, meta, recType);
};
Ext.extend(Ext.ux.data.GbFileAttrJsonReader, Ext.data.JsonReader, {
    ...
    translateResponse : function(jsonObj) {
      ...
      var attrHash = jsonObj.data.attributes ;
      var attrArr = [] ;
      for(attrName in attrHash)
      {
        /* Double encode the id because it will be used in the rsrcPath URL parameter to API caller */
        attrArr.push({'id': encodeURIComponent(encodeURIComponent(attrName)), 'attrName': attrName, 'attrValue': attrHash[attrName]}) ;
      }
      jsonObj.data = attrArr;
      return jsonObj
    },
...
Ext.ux.data.GbFileAttrHttpProxy = function(meta, recType) {
    // call parent constructor
    Ext.ux.data.GbFileAttrHttpProxy.superclass.constructor.call(this, meta, recType);
};
Ext.extend(Ext.ux.data.GbFileAttrHttpProxy, Ext.data.HttpProxy, {
    ...
    translateForUpdate : function(reqBody)
    {
      ...
      var attrApiObj = {data:{attributes:{}}} ;
      attrApiObj.data.attributes[attrHash.attrName] = attrHash.attrValue ;
...
var proxy = new Ext.ux.data.GbFileAttrHttpProxy({
    api: {
        read : 'apiCaller.jsp?rsrcPath='+encodeURIComponent('/REST/v1/grp/'+encGrpName+'/db/'+encDbName+'/file/'+encFileName+'/attributes'),
        create : {url: 'apiCaller.jsp?apiMethod=PUT&rsrcPath='+encodeURIComponent('/REST/v1/grp/'+encGrpName+'/db/'+encDbName+'/file/'+encFileName+'/attribute'), method: 'POST' },
        update: {url: 'apiCaller.jsp?apiMethod=PUT&rsrcPath='+encodeURIComponent('/REST/v1/grp/'+encGrpName+'/db/'+encDbName+'/file/'+encFileName+'/attribute'), method: 'POST' },
        destroy: {url: 'apiCaller.jsp?apiMethod=DELETE&rsrcPath='+encodeURIComponent('/REST/v1/grp/'+encGrpName+'/db/'+encDbName+'/file/'+encFileName+'/attribute'), method: 'POST' }
    }
    
});

// Typical JsonReader.  Notice additional meta-data params for defining the core attributes of your json-response
var reader = new Ext.ux.data.GbFileAttrJsonReader({
    idProperty: 'id',
    root: 'data',
//    successProperty: 'status.statusCode',
    successProperty: function(resp) { return (resp.status.statusCode == 'OK') ? true : false ; },
