...
<%
		  ...
		  else
	{
	  ...
	  if (dbName == null) {
		  GenboreeMessage.setErrMsg(mys, "There is no database");
	  ...
	  if (mys.getAttribute("localftypeids") != null) {
		...
		//out.println("<br> 370  restrict region " + restrictRegion );
	 ...
	 //out.println("<br>378   "   +shareftypeids);
	  ...
	  if (mys.getAttribute("totalNumAnnotations_VGA") != null) {
		 ...
		 GenboreeMessage.setErrMsg(mys, " error 419");
			  GenboreeUtils.sendRedirect(request, response, "/java-bin/displaySelection.jsp?fromBrowser=View");
			  return;
	  ...
	  if (initPage) {
		  ...
		  if (totalNumAnnotations > 0 || !initPage) {
		   ...
		   //out.println("<br> 500 is large sample    "  + isLargeSamples   );
	 	...
	 	if (shareAnnoGrp != null) {
			  ...
			  if (localftypeids != null) {
				  AnnotationGroup localGroup = GroupHelper.retrieveLargeAnnoGroupByChromosomalRegion(localftypeids, localftype2chromsomeMap, con, rid, chrStart, chrStop, out);
			  ...
			  if (shareftypeids != null) {
				  AnnotationGroup sharedGroup = GroupHelper.retrieveLargeAnnoGroupByChromosomalRegion(shareftypeids, shareftype2chromsomeMap, sharedConnection, rid, chrStart, chrStop, out);
		  ...
		  //out.println("<br>1031 shared " +  shareftypeids  + " num shared " + sharedGrpAnnotations  );

		  if (sortingColumns) {
			  //out.println("<br>line 1033 " +   totalNumGrpAnnotations   +"  is large samples " +  isLargeSamples + "  has data " + hasData  );

			  if (!isLargeSamples && hasData) {
				  //out.println("<br>line 1001 " +   totalNumGrpAnnotations );
				  AnnotationGroup localAnnoGrp = null;
				  if (localftypeids != null && localftypeids.length > 0)
		  localAnnoGrp = GroupHelper.retrieveGroupAnnotationsByChromosomeRegion (localftypeids, rid , chrStart, chrStop, con, out);
				  int localgrpNum = 0;
				  if (localAnnoGrp != null) {
					  if (localAnnoGrp.getAnnos() != null)
						  localGrpAnnotations = localAnnoGrp.getAnnos();
				  ...
				  AnnotationGroup shareAnnoGrp = null;
				  if (shareftypeids != null && shareftypeids.length > 0)
					  shareAnnoGrp = retrieveLargeAnnoGroupByChromosomalRegion(shareftypeids, shareftype2chromsomeMap, sharedConnection, rid, chrStart, chrStop, out);
							  int sharegrpnum = 0;
				  if (shareAnnoGrp != null) {
					  if (shareAnnoGrp.getAnnos() != null)
						  sharedGrpAnnotations = shareAnnoGrp.getAnnos();
				  ...
				  if (localftypeids != null) {
					  AnnotationGroup localGroup = retrieveLargeAnnoGroupByChromosomalRegion(localftypeids, localftype2chromsomeMap, con, rid, chrStart, chrStop, out);
								  localGrpAnnotations = localGroup.getAnnos();
					  localGroupName2fids = localGroup.getGroupName2Fids();
					...
					//  return;
					   ...
					   localGrpAnnotations = null;     localGroupName2fids = null;
				  }
				   mys.setAttribute("localGrpAnnotations", localGrpAnnotations);
					  mys.setAttribute("localGroupName2fids", localGroupName2fids);
				  ...
				  if (shareftypeids != null) {
					  AnnotationGroup sharedGroup = retrieveLargeAnnoGroupByChromosomalRegion(shareftypeids, shareftype2chromsomeMap, sharedConnection, rid, chrStart, chrStop, out);
					  sharedGrpAnnotations = sharedGroup.getAnnos();
					  sharedGroupName2fids = sharedGroup.getGroupName2Fids();
				  ...
				  else {sharedGrpAnnotations = null;
				  ...
				  //out.println("<br>1152 before  merge   " +  totalAnnotations  +  " local " +   localGrpAnnotations.length  + " shared " +  sharedGrpAnnotations );
				  ...
				  AnnotationDetail[] tempTotal = new AnnotationDetail[totalNumGrpAnnotations];
				          ...
				          //out.println("<br>1154 arr length    " + totalNumGrpAnnotations);
				...
				//  totalAnnotations = GroupHelper.mergeAnnotations(localGrpAnnotations, sharedGrpAnnotations);
				  //out.println("<br>1178 afeter merge    " +  totalAnnotations);
			...
			totalAnnotations = OneAttAnnotationSorter.sortAllAnnotations(sortingColumnName, totalAnnotations, sortingOrder);
			  ...
			  paging = true;
			  //out.println("<br> line 1446   "  + totalAnnotations.length );
...
<%@ include file="include/header.incl" %>
  <%@ include file="include/navbar.incl" %>
	  ...
	  <form name="viewForm" id="viewForm" action="viewGroupAnnotationsByRegion.jsp" method="post" >
