...
<%
  ...
  String urlStr = GenboreeUtils.returnFullURL(request, "/genboree/toolPlugins/downloadResults.rhtml");
...
%>
  ...
  <%  // TEMPLATE: grab the external page and present its contents:
      ...
      if(hdrsMap.containsKey("X-GENBOREE-GROUP"))
        out.write("<!-- Found X-GENBOREE-GROUP: '" + ((String)((List)hdrsMap.get("X-GENBOREE-GROUP")).get(0)) + "' -->") ;
      if(hdrsMap.containsKey("X-GENBOREE-DATABASE"))
        out.write("<!-- Found X-GENBOREE-DATABASE: '" + ((String)((List)hdrsMap.get("X-GENBOREE-DATABASE")).get(0)) + "' -->") ;

      // Set the curr group/db if changed by the tool
      boolean updatedSeesion = GenboreeUtils.setContextFromHeaders(hdrsMap, mys) ;
